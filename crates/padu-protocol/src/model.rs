...
///
/// Drafts may carry [`Self::NewWorktree`] until their first prompt. Padu then
...
pub enum DriverEvent {
    ...
    /// fresh Harness session may resolve its deployment default when Padu did
...
mod tests {
    ...
    fn attachment_messages_keep_transport_and_visible_content_separate() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("padu-blob:ab/reference.png".to_owned()),
        };
    ...
    fn activity_targets_are_normalized_when_events_arrive() {
        let cases = [
            (
                ActivityKind::FileRead,
                serde_json::json!({"input": {"file_path": "/tmp/padu/src/app.rs"}}),
                "/tmp/padu/src/app.rs",
            ),
            (
                ActivityKind::FileSearch,
            ...
            (
                ActivityKind::FileList,
                serde_json::json!({"arguments": {"directory": "/tmp/padu/src"}}),
                "/tmp/padu/src",
            ),
            (
            ...
            (
                ActivityKind::Search,
                serde_json::json!({"action": {"queries": ["Padu GPUI"]}}),
                "Padu GPUI",
            ),
            (
                ActivityKind::FileRead,
                serde_json::json!("/tmp/padu/README.md"),
                "/tmp/padu/README.md",
            ),
        ];
    ...
    fn projectless_projects_use_projects_root_and_recognize_legacy_paths() {
        ...
        let ordinary = Project::from_path(home.join("dev/padu"));
    ...
    fn legacy_file_edit_details_are_promoted_to_arguments_and_metadata() {
        ...
        assert_eq!(activities[0].file_changes[0].path, "/tmp/padu/README.md");
    ...
    fn legacy_file_tools_are_reclassified_and_gain_cached_targets() {
        ...
        cached.display_target = Some("/tmp/padu/src/persisted.rs".into());
        session.transcript_blocks.push(TranscriptBlock {
            after_message: 0,
            turn_id: None,
            activities: vec![
                ActivityItem::new(
                    ...
                    ActivityKind::Search,
                    "read",
                    Some(r#"{"filePath":"/tmp/padu/src/model.rs"}"#.into()),
                    true,
                ),
                ActivityItem::new(
                    None,
        ...
        assert_eq!(
            ...
            Some("/tmp/padu/src/persisted.rs")
        );
        assert!(
            activities[..2]
                .iter()
