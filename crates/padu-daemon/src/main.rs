...
fn main() -> anyhow::Result<()> {
    ...
    let token =
        std::env::var(DAEMON_TOKEN_ENV).context("Padu daemon authentication token is missing")?;
    ...
    let task_path = padu_core::persistence::StateStore::default_path();
    let settings = padu_core::DaemonSettingsStore::open_with_legacy(
        padu_core::DaemonSettings::default_path(),
    ...
    padu_core::serve(
        listener,
        token,
        Arc::new(padu_core::daemon::PaduBackend::new(settings, task_store)?),
        shutdown,
        padu_core::ServerOptions {
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.padu.test", "http://localhost:3000"]
        );
