...
use orbis_protocol::MAX_WIRE_MESSAGE_BYTES;
use orbis_protocol::{
...
impl DaemonClient {
    ...
    pub fn connect_with_resume(
        ...
        let (mut socket, _) =
            tungstenite::client::connect_with_config(url.as_str(), Some(config), 3)
                .context("could not connect to Orbis daemon")?;
        ...
        std::thread::Builder::new()
            .name("orbis-daemon-client".into())
            .spawn(move || run_client(socket, outgoing_rx, thread_inner))
            .context("could not start Orbis daemon client thread")?;
        Ok(Self { inner })
    }
    ...
    pub fn request(
        ...
        if self.inner.disconnected.load(Ordering::Acquire) {
            bail!("Orbis daemon is disconnected");
        }
            ...
            .is_err()
        {
            ...
            bail!("Orbis daemon connection is closed");
        }
        match response_rx.recv_timeout(REQUEST_TIMEOUT) {
            ...
            Err(error) => {
                self.inner.pending.lock().remove(&request_id);
                Err(anyhow!("timed out waiting for Orbis daemon: {error}"))
            }
        }
    }
    ...
    pub fn notify(
        ...
        self.inner
            ...
            .map_err(|_| anyhow!("Orbis daemon connection is closed"))
    }
...
fn daemon_url(address: &str) -> anyhow::Result<String> {
    ...
    let mut url = url::Url::parse(&normalized).context("Orbis daemon address is invalid")?;
    url.set_path("/v1");
    url.set_query(None);
...
fn run_client(
    ...
    for (_, response) in pending {
        let _ = response.send(Err(RpcError {
            message: "Orbis daemon disconnected".into(),
        }));
    }
...
fn read_server_message(
    ...
    loop {
        match socket.read()? {
            ...
            Message::Close(_) => bail!("Orbis daemon closed during handshake"),
            _ => {}
        }
    }
}
...
mod tests {
    ...
    fn daemon_endpoint_accepts_addresses_and_secure_urls() {
        ...
        assert_eq!(
            daemon_url("wss://orbis.example.test/old?ignored=1").unwrap(),
            "wss://orbis.example.test/v1"
        );
    }
}
