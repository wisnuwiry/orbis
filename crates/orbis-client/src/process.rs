...
impl DaemonProcess {
    ...
    fn spawn_configured(
        ...
        let app_executable = std::env::current_exe().context("could not locate Orbis executable")?;
        ...
        let stdout = child
            .stdout
            .take()
            .context("Orbis daemon did not expose its readiness stream")?;
        let (ready_tx, ready_rx) = mpsc::sync_channel(1);
        std::thread::Builder::new()
            .name("orbis-daemon-ready".into())
            .spawn(move || {
                ...
                let result = BufReader::new(stdout)
                    ...
                    .and_then(|bytes| {
                        if bytes == 0 {
                            bail!("Orbis daemon exited before becoming ready")
                        }
                        serde_json::from_str::<DaemonReady>(&line).map_err(anyhow::Error::from)
                    });
                let _ = ready_tx.send(result);
            })
            .context("could not start Orbis daemon readiness reader")?;
        let ready = match ready_rx.recv_timeout(START_TIMEOUT) {
            ...
            Err(error) => {
                let _ = child.kill();
                let _ = child.wait();
                bail!("timed out waiting for Orbis daemon: {error}");
            }
        };
...
fn desktop_client_address(address: &str) -> anyhow::Result<String> {
    let address = address
        ...
        .with_context(|| format!("Orbis daemon returned an invalid address {address:?}"))?;
...
impl DaemonSupervisor {
    ...
    pub fn spawn_configured(
        ...
        std::thread::Builder::new()
            .name("orbis-daemon-supervisor".into())
            .spawn(move || monitor_daemon(weak_inner, Some(initial_stamp), watch_for_rebuilds))
            .context("could not start Orbis daemon supervisor")?;
        Ok(supervisor)
    }
    ...
    pub fn connect(address: &str, token: String) -> anyhow::Result<Self> {
        ...
        let weak_inner = Arc::downgrade(&supervisor.inner);
        std::thread::Builder::new()
            .name("orbis-remote-daemon-supervisor".into())
            .spawn(move || monitor_daemon(weak_inner, None, false))
            .context("could not start remote Orbis daemon supervisor")?;
        Ok(supervisor)
    }
    ...
    fn from_target(
        ...
        std::thread::Builder::new()
            .name("orbis-daemon-settings".into())
            .spawn(move || persist_settings(weak_inner, settings_update_rx))
            .context("could not start Orbis daemon settings writer")?;
        Ok(Self { inner })
    }
    ...
    pub fn reconfigure(&self, exposure: DaemonExposureSettings) -> anyhow::Result<()> {
        ...
        let executable = self
            ...
            .executable
            .as_ref()
            .context("the connected daemon is managed outside Orbis Desktop")?
            .clone();
    ...
    pub fn update_settings(&self, settings: DaemonSettings) -> anyhow::Result<()> {
        *self.inner.settings.lock() = settings.clone();
        ...
        self.inner
            .settings_updates
            .send(settings)
            .map_err(|_| anyhow::anyhow!("Orbis daemon settings writer is closed"))
    }
}
...
fn monitor_daemon(
    ...
    loop {
        ...
        match replace_local_daemon(&inner, executable, &exposure) {
            Ok(()) => {}
            Err(error) => {
                eprintln!("could not restart rebuilt Orbis daemon: {error:#}");
                continue;
            }
        }
...
fn replace_local_daemon(
    ...
    let previous = {
        ...
        match &*target {
            DaemonTarget::Remote { .. } => {
                bail!("the connected daemon is managed outside Orbis Desktop")
            }
...
fn read_settings(client: &DaemonClient) -> anyhow::Result<DaemonSettings> {
    match client.request(Uuid::nil(), Uuid::nil(), Command::GetSettings)? {
        ...
        _ => bail!("Orbis daemon returned an invalid settings response"),
    }
}
    ...
    while let Ok(mut settings) = updates.recv() {
        ...
        loop {
            ...
            match result {
                ...
                Ok(_) => {
                    eprintln!("Orbis daemon returned an invalid settings update response");
                }
                Err(error) => {
                    eprintln!("could not persist Orbis daemon settings: {error:#}");
                }
            }
            drop(inner);
            std::thread::sleep(REBUILD_POLL_INTERVAL);
    ...
    fn browser_origins_are_exact_and_deduplicated() {
        assert_eq!(
            parse_allowed_origins(
                "https://app.orbis.test, http://localhost:3001, https://app.orbis.test"
            )
            .unwrap(),
            ["https://app.orbis.test", "http://localhost:3001"]
        );
        assert!(parse_allowed_origins("https://app.orbis.test/path").is_err());
        assert!(parse_allowed_origins("ws://app.orbis.test").is_err());
    }
