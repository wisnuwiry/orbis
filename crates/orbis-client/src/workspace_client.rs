...
use orbis_protocol::{Command, ResponsePayload, WorkspaceOperation, WorkspaceResult};
...
impl WorkspaceClient {
    ...
    pub fn request(&self, operation: WorkspaceOperation) -> anyhow::Result<WorkspaceResult> {
            ...
            .request(Uuid::nil(), Uuid::nil(), Command::Workspace { operation })?
        {
            ...
            _ => anyhow::bail!("Orbis daemon returned an invalid workspace response"),
        }
    }
    ...
    pub fn fork_provider_session(
        ...
        match self.client.request(
            ...
            _ => anyhow::bail!("Orbis daemon returned an invalid provider-fork response"),
        }
    }
}
