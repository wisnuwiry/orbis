...
use orbis_protocol::computer_use::ComputerAppGrant;
use orbis_protocol::i18n::AppLanguage;
use orbis_protocol::identity::DATA_DIRECTORY_NAME;
use orbis_protocol::model::{
...
use orbis_protocol::theme::ThemePreference;

pub use orbis_protocol::persistence::{
...
impl ComposerDraftStore {
    ...
    pub fn load(&self) -> io::Result<ComposerDrafts> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Orbis daemon returned an invalid composer-drafts response",
            )),
        }
    }
    ...
    pub fn save(&self, drafts: ComposerDrafts, generation: u64) -> io::Result<()> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Orbis daemon returned an invalid composer-drafts save response",
            )),
        }
    }
}
    ...
    fn migrate_loaded(&mut self) {
        for session in &mut self.sessions {
            let checkpoint_totals_current = session.turns.iter().all(|turn| {
                turn.checkpoint
                    .as_ref()
                    .is_none_or(orbis_protocol::model::Checkpoint::totals_are_current)
            });
            let before = (
                session.turns.len(),
...
impl StateStore {
    ...
    pub fn session_message_search(
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Orbis daemon returned an invalid message-search response",
            )),
        }
    }
    ...
    pub fn provider_sessions(
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Orbis daemon returned an invalid provider-session response",
            )),
        }
    }
    ...
    pub fn provider_session_history(
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Orbis daemon returned an invalid provider-session history response",
            )),
        }
    }
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                orbis_protocol::projectless::set_workspace_root(projectless_root);
                (projects, sessions, default_cwd)
            }
            _ => {
                return Err(io::Error::other(
                    "Orbis daemon returned an invalid task-state response",
                ));
            }
        };
...
pub fn hydrate_session(
        ...
        .map_err(to_io_error)?
    {
        ...
        _ => Err(io::Error::other(
            "Orbis daemon returned an invalid session-hydration response",
        )),
    }
}
...
pub fn read_remote_reference(
    ...
    let command = if orbis_protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(orbis_protocol::attachments::ATTACHMENT_SCHEME) {
