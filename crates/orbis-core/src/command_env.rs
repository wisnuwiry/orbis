...
#[cfg(unix)]
const SHELL_ENV_COMMAND: &str = "/usr/bin/env -0 > \"$ORBIS_SHELL_ENV_CAPTURE_FILE\"";
...
/// Build a command with the environment a terminal-launched Orbis normally
...
/// The `PATH` a provider CLI runs with: every directory Orbis itself searched,
...
/// GUI-launched Orbis can still be running with only the `PATH` it inherited.
...
///
/// Orbis's Windows build is a GUI-subsystem binary with no console of its own,
...
/// Normalize a Orbis-owned provider thread before a dependency spawns the child
...
/// Orbis inherits explorer's `PATH`, which predates later installs, and the
...
foreach ($target in @('User', 'Machine')) {
  ...
  if ($value) { $entries.Add('ORBIS_' + $target.ToUpper() + '_PATH=' + [Environment]::ExpandEnvironmentVariables($value)) }
}
[IO.File]::WriteAllText($env:ORBIS_SHELL_ENV_CAPTURE_FILE, [string]::Join([string][char]0, $entries))
";
...
fn capture_windows_environment(
    ...
    command
        ...
        .env("ORBIS_SHELL_ENV_CAPTURE_FILE", capture.path())
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn merge_windows_environment(mut environment: ShellEnvironment) -> Option<ShellEnvironment> {
    ...
    for name in ["PATH", "ORBIS_USER_PATH", "ORBIS_MACHINE_PATH"] {
...
pub fn default_terminal_shell() -> PathBuf {
    ...
    // see the note in orbis-client's `unix_terminal_shell_candidates`. The
...
impl ShellEnvironmentCapture {
    fn create() -> Option<Self> {
        for _ in 0..16 {
            ...
            let path =
                std::env::temp_dir().join(format!(".orbis-shell-env-{}-{id}", std::process::id()));
...
mod tests {
    ...
    fn a_provider_cli_runs_with_the_directories_detection_searched() {
        #[cfg(windows)]
        let program = PathBuf::from("C:\\orbis-fixture\\bin\\pi.exe");
        #[cfg(not(windows))]
        let program = PathBuf::from("/opt/orbis-fixture/bin/pi");
    ...
    fn spawn_unblocks_sigchld_in_the_child_and_restores_the_caller() {
        if std::env::var_os("ORBIS_SIGCHLD_CHILD_PROBE").is_some() {
        ...
        command
            ...
            .env("ORBIS_SIGCHLD_CHILD_PROBE", "1");
    ...
    fn windows_profile_environment_merges_registry_paths_behind_the_profile_path() {
        let environment = merge_windows_environment(vec![
            ...
            (
                OsString::from("ORBIS_USER_PATH"),
                OsString::from("C:\\user;C:\\shared"),
            ),
            (
                OsString::from("ORBIS_MACHINE_PATH"),
        ...
        assert!(
            !environment
                .iter()
                .any(|(name, _)| name == OsStr::new("ORBIS_USER_PATH"))
        );
        assert!(
            !environment
                .iter()
                .any(|(name, _)| name == OsStr::new("ORBIS_MACHINE_PATH"))
        );
    }
    ...
    fn a_bare_name_resolves_through_pathext() {
        let directory = std::env::temp_dir().join(format!("orbis-pathext-{}", std::process::id()));
    ...
    fn a_bare_name_prefers_pathext_over_an_extensionless_shim() {
        ...
        let directory =
            std::env::temp_dir().join(format!("orbis-pathext-shim-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&directory);
        std::fs::create_dir_all(&directory).expect("create fixture directory");
    ...
    fn parses_null_delimited_environment_without_losing_value_contents() {
        let environment = parse_shell_environment(
            b"PATH=/Users/example/.fnm/current/bin:/usr/bin\0TOKEN=line one\nline two=rest\0EMPTY=\0ORBIS_SHELL_ENV_CAPTURE_FILE=/tmp/capture\0",
        )
    ...
    fn captures_environment_from_a_shell_process() {
        let id = SHELL_ENV_CAPTURE_ID.fetch_add(1, Ordering::Relaxed);
        let directory =
            std::env::temp_dir().join(format!("orbis-command-env-test-{}-{id}", std::process::id()));
        fs::create_dir(&directory).expect("create shell fixture directory");
        let shell = directory.join("fake-shell");
        fs::write(
            &shell,
            "#!/bin/sh\n/usr/bin/printf 'PATH=/Users/example/.fnm/current/bin:/usr/bin\\000ORBIS_TEST_TOKEN=from-shell\\000' > \"$ORBIS_SHELL_ENV_CAPTURE_FILE\"\n",
        )
        ...
        assert_eq!(
            ...
            vec![
                ...
                (
                    OsString::from("ORBIS_TEST_TOKEN"),
