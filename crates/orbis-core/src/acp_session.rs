...
//! reading their private stores, while typed updates let Orbis discard private
...
pub(crate) fn catalog_working_directory() -> anyhow::Result<PathBuf> {
    let directory = std::env::temp_dir().join(format!(
        "orbis-provider-session-catalog-{}",
        std::process::id()
    ));
...
pub fn list_provider_sessions(
    ...
    let request = Client.builder().name("orbis-session-catalog").connect_with(
        agent,
