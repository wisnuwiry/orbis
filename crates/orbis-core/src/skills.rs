...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("orbis-skills-{tag}-{}", std::process::id()));
    ...
    fn scan_orders_user_before_projects_and_counts_cross_scope_duplicates() {
        ...
        assert_eq!(
            names,
            vec![("review", None), ("zeta", None), ("review", Some("orbis"))]
        );
    ...
    fn every_ecosystem_root_is_listed() {
        let project_root = std::env::temp_dir().join("orbis-skills-project");
        let projects = vec![("orbis".to_owned(), project_root.clone())];
