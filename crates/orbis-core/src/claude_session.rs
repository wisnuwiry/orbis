...
/// conversation on the next Orbis prompt.
...
fn list_provider_sessions_in(
    ...
    // from this index, which naturally keeps Orbis-created sessions out of the
    // terminal resume picker even after their Orbis task has been deleted.
...
fn fork_session_at_in(
    ...
    output.push(json!({
        "type": "custom-title",
        "sessionId": forked_session_id,
        "customTitle": if title.trim().is_empty() { "Orbis rewind" } else { title.trim() },
        "uuid": Uuid::new_v4().to_string(),
        "timestamp": now,
    }));
...
mod tests {
    ...
    fn fixture() -> (PathBuf, PathBuf) {
        let root = std::env::temp_dir().join(format!("orbis-claude-session-{}", Uuid::new_v4()));
