...
//! empty answer and calls it a success. Reading the wire log is what lets Orbis
...
/// permission dialogs just by opening Orbis's Resume picker.
...
/// hash Orbis cannot reproduce, so the session id is matched by scanning.
...
mod tests {
    ...
    fn lists_kimi_owned_state_without_opening_the_recorded_workspace() {
        let root = std::env::temp_dir().join(format!("orbis-kimi-catalog-{}", uuid::Uuid::new_v4()));
        let session = root.join("sessions/wd-protected/session-native");
        let recorded_cwd = std::env::temp_dir().join(format!(
            "orbis-kimi-protected-workspace-{}",
            uuid::Uuid::new_v4()
        ));
