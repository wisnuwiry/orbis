...
/// Orbis-originated `thread/goal/*` requests awaiting their responses, plus
...
/// Register Orbis's long-lived QuickJS MCP server and keep the raw native helper
...
fn configure_computer_use_command(command: &mut Command, config: Option<&CodexComputerUseConfig>) {
    if let Some(config) = config {
        command
            ...
            .env("ORBIS_COMPUTER_USE_SERVER", &config.server_path)
            .env(
                "ORBIS_COMPUTER_USE_PROCESS_DIRECTORY",
                &config.process_directory,
            )
            .arg("-c")
            .arg(format!("mcp_servers.orbis_js_repl.command={}", config.repl))
            .arg("-c")
            .arg("mcp_servers.orbis_js_repl.args=[]")
            .arg("-c")
            .arg(format!(
                "mcp_servers.orbis_js_repl.env.ORBIS_COMPUTER_USE_SERVER={}",
                config.server
            ))
            .arg("-c")
            .arg(format!(
                "mcp_servers.orbis_js_repl.env.ORBIS_COMPUTER_USE_PROCESS_DIRECTORY={}",
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if let Some(computer_use_skill_root) = computer_use_skill_root {
                    // Register Orbis's bundled skill through Codex's discoverable-skill
                ...
                while let Ok(command) = command_rx.recv() {
                    let message = match command {
                        ...
                        CommandMessage::GeneratedTitle(title) => {
                            ...
                            // Update Orbis even if persisting the name back to
...
fn turn_start_params(
    ...
    json!({
        ...
        // Some current models default reasoning summaries to `none`. Orbis has
...
fn generate_codex_title(binary: &Path, cwd: &Path, prompt: &str) -> anyhow::Result<String> {
    ...
    let reader = thread::Builder::new()
        .name("orbis-codex-title-reader".into())
        .spawn(move || {
...
fn handle_codex_message(
    ...
    // the same numeric ID as one of Orbis's earlier requests. Only messages
    // without a method are responses to Orbis-originated requests.
...
mod tests {
    ...
    fn computer_use_command_configuration_follows_the_setting() {
        ...
        assert!(
            disabled
                .get_envs()
                .all(|(name, _)| { !name.to_string_lossy().starts_with("ORBIS_COMPUTER_USE_") })
        );

        let config = CodexComputerUseConfig {
            server_path: PathBuf::from("/tmp/orbis-computer-use-server"),
            server: toml_string("/tmp/orbis-computer-use-server"),
            repl: toml_string("/tmp/orbis"),
            skill_root: PathBuf::from("/tmp/orbis-computer-use-skill"),
            process_directory: PathBuf::from("/tmp/orbis-computer-use-processes"),
            process_directory_config: toml_string("/tmp/orbis-computer-use-processes"),
        };
        ...
        // Orbis REPL owns it and exposes only `sky` inside JavaScript.
        assert!(
            !enabled_arguments
                .iter()
                .any(|argument| argument.contains("mcp_servers.orbis_computer_use"))
        );
        assert!(
            enabled_arguments
                .iter()
                .any(|argument| argument.contains("mcp_servers.orbis_js_repl.command"))
        );
        assert!(
            enabled_arguments
                .iter()
                .any(|argument| { argument == "mcp_servers.orbis_js_repl.args=[]" })
        );
        assert!(
            enabled_arguments
                .iter()
        ...
        assert!(
            enabled
                .get_envs()
                .any(|(name, _)| { name.to_string_lossy() == "ORBIS_COMPUTER_USE_SERVER" })
        );
    }
    ...
    fn computer_use_process_registry_accepts_only_pid_files() {
        let directory = std::env::temp_dir().join(format!(
            "orbis-computer-use-process-test-{}",
            Uuid::new_v4().simple()
        ));
    ...
    fn mcp_file_tools_use_the_shared_file_presentation_metadata() {
        let item = json!({
            ...
            "server": "filesystem",
            "tool": "read_file",
            "arguments": {"path": "/tmp/orbis/src/model.rs"},
            "status": "inProgress"
        });
    ...
    fn command_actions_use_semantic_file_presentation() {
        for (action, expected_kind, expected_title, expected_target) in [
            (
                json!({
                    "type": "read",
                    "command": "sed -n '12,20p' crates/orbis-core/src/driver/codex.rs",
                    "name": "codex.rs",
                    "path": "/tmp/orbis/crates/orbis-core/src/driver/codex.rs"
                }),
                ActivityKind::FileRead,
                tr!("activity.read_file"),
                "/tmp/orbis/crates/orbis-core/src/driver/codex.rs",
            ),
            (
                json!({
                    "type": "listFiles",
                    "command": "rg --files crates/orbis-core/src",
                    "path": "crates/orbis-core/src"
                }),
                ActivityKind::FileList,
                tr!("activity.list_files"),
                "crates/orbis-core/src",
            ),
            (
                json!({
                    "type": "search",
                    "command": "rg commandActions crates/orbis-core/src",
