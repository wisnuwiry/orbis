...
//! permission requests the user can actually be asked. Orbis already started
...
impl OpenCodeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        // Orbis must therefore install the selected access policy on this
        ...
        thread::Builder::new()
            .name("orbis-opencode-usage-metadata".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("orbis-opencode-events".into())
            .spawn(move || {
                ...
                match open_event_stream(stream_port, "/event", &stream_control) {
                    Ok(Some(stream)) => {
                        ...
                        // lost when Orbis reconnects after it was asked. Events
        ...
        thread::Builder::new()
            .name("orbis-opencode-driver".into())
            .spawn(move || {
...
fn rehydrate_pending_permissions(
    ...
    // `session.idle` settles Orbis's persisted running turn exactly once.
...
fn request_permission(
    ...
    // so Orbis sends only one-shot provider replies and retains durable choices
...
mod tests {
    ...
    /// `cargo test --bin orbis opencode_session_against_a_real_server -- --ignored`.
    #[test]
    ...
    fn permission_approvals_stay_driver_local() {
        ...
        let repeated = json!({
            ...
            "properties": {
                ...
                "sessionID": "ses_1",
                "permission": "bash",
                "patterns": ["rm -rf /tmp/orbis-cache"],
                "metadata": {},
                "always": ["rm -rf *"]
            }
        });
