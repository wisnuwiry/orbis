...
//! unknown-method errors, stdio lifetime, and protocol type validation. Orbis
...
fn launch_for(provider: ProviderKind, reasoning_effort: Option<&str>) -> anyhow::Result<AcpLaunch> {
    match provider {
        ...
        ProviderKind::Grok => {
            ...
            Ok(AcpLaunch {
                args,
                env: vec![("GROK_OAUTH2_REFERRER".into(), "orbis".into())],
            })
        }
...
impl AcpDriver {
    pub fn start(
        ...
        thread::Builder::new()
            .name(format!("orbis-{}-acp", provider.id()))
            .spawn(move || {
    ...
    Client
        ...
        .connect_with(agent, async move |connection: ConnectionTo<Agent>| {
            ...
            if provider == ProviderKind::Cursor {
                ...
                // clients that opt in. Orbis applies the returned config option
            ...
            let initialize = connection
                .send_request(
                    InitializeRequest::new(ProtocolVersion::V1)
                        ...
                        .client_info(Implementation::new("orbis", env!("CARGO_PKG_VERSION"))),
                )
                .block_task()
                .await?;
...
/// agents Orbis drives keep it on `mode`. Grok does not use this path: its
...
fn send_prompt(
    ...
    let extension_id =
        (provider == ProviderKind::Grok).then(|| format!("orbis-{}", uuid::Uuid::new_v4()));
...
fn handle_session_update(
    ...
    match kind {
        ...
        // `user_message_chunk` is Orbis's own prompt echoed back. Other typed
...
mod tests {
    ...
    fn typed_updates_preserve_text_reasoning_and_correlated_tools() {
        ...
        let updates = [
            ...
            json!({"sessionUpdate":"tool_call_update","toolCallId":"call_1","status":"completed","title":"fixture.txt","content":[{"type":"content","content":{"type":"text","text":"orbis probe fixture"}}]}),
        ...
        assert!(matches!(&seen[2], DriverEvent::RichActivity(item)
                if item.complete
                    ...
                    && item.output.as_deref().is_some_and(|output| output.contains("orbis probe fixture"))));
