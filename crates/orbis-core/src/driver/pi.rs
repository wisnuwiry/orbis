...
/// Oh My Pi refuses to reassemble beyond this, so neither should Orbis.
...
impl PiFlavor {
    ...
    /// Oh My Pi does have one, and Orbis only ever runs these in Full access.
    ...
    /// Orbis's computer-use bridge is a Pi extension written against Pi's
    /// extension API. Oh My Pi ships its own `/computer` instead.
    fn supports_orbis_computer_use(self) -> bool {
        matches!(self, Self::Pi)
    }
...
fn configure_pi_computer_use_command(
    ...
    if let Some((config, extension)) = config {
        command
            ...
            .env("ORBIS_JS_REPL_SERVER", &config.repl_path)
...
impl PiDriver {
    pub fn start(
        ...
        let computer_use = (computer_use_enabled && flavor.supports_orbis_computer_use())
...
impl DriverControl for PiDriver {
    ...
    fn apply_options(&self, options: SessionOptions) -> bool {
        ...
        // Orbis only runs them in Build with Full access anyway — so a mode
...
fn send_request(
    ...
    *next_request_id += 1;
    let id = format!("orbis-{}", next_request_id);
...
/// Keep Orbis's meter aligned with that provider-native calculation.
...
fn pi_fork_request(
    ...
    if turns_to_remove > messages.len() {
        return Err(format!(
            "{name} has only {} native turns, but Orbis needs to remove {turns_to_remove}",
            messages.len()
        ));
    }
...
fn clone_ohmypi_session(
    ...
    let result = (|| {
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                for line in BufReader::new(stdout).lines().map_while(Result::ok) {
                    ...
                    if value.get("id").and_then(Value::as_str) == Some("orbis-clone") {
                        let _ = tx.send(value);
                        break;
                    }
                }
        ...
        write_json_line(
            &mut stdin,
            &json!({"id": "orbis-clone", "type": "get_state"}),
        )
...
mod tests {
    ...
    fn pi_computer_use_uses_only_session_scoped_extension_and_skill_arguments() {
        let config = computer_use_runtime::ComputerUseConfig {
            server_path: PathBuf::from("/tmp/Orbis Computer Use"),
            repl_path: PathBuf::from("/Applications/Orbis.app/Resources/orbis_js_repl"),
            skill_path: PathBuf::from("/Applications/Orbis.app/Resources/skills/SKILL.md"),
            process_directory: PathBuf::from("/tmp/orbis-computer-use/session"),
        };
        ...
        configure_pi_computer_use_command(
            ...
            Some((
                &config,
                Path::new("/Applications/Orbis.app/Resources/computer-use/pi-extension.ts"),
            )),
        );
        ...
        assert_eq!(
            ...
            [
                "--extension",
                "/Applications/Orbis.app/Resources/computer-use/pi-extension.ts",
                "--skill",
                "/Applications/Orbis.app/Resources/skills/SKILL.md",
            ]
        );
        let environment = command
            ...
            .collect::<HashMap<_, _>>();
        assert_eq!(
            environment.get("ORBIS_JS_REPL_SERVER"),
            Some(&Some(
                "/Applications/Orbis.app/Resources/orbis_js_repl".into()
            ))
        );
        assert_eq!(
            environment.get("ORBIS_COMPUTER_USE_PROCESS_DIRECTORY"),
            Some(&Some("/tmp/orbis-computer-use/session".into()))
        );
    }
    ...
    fn chunked_frames_reassemble_and_reject_broken_runs() {
        ...
        let payload = json!({"type": "response", "id": "orbis-1", "success": true});
        ...
        // A run that starts mid-sequence is not a frame Orbis can trust.
        let mut assembly = ChunkAssembly::default();
