...
fn stop_task_request(request_id: u64, task_id: &str) -> Value {
    json!({
        ...
        "request_id": format!("orbis-{request_id}"),
...
impl ClaudeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
                    ...
                    let written = match message {
                        ...
                        CommandMessage::Cancel => {
                            ...
                            write_line(
                                ...
                                &json!({
                                    "type": "control_request",
                                    "request_id": format!("orbis-{next_request_id}"),
                        ...
                        CommandMessage::StopBackgroundWork { key, control_id } => {
                            next_request_id += 1;
                            let request_id = format!("orbis-{next_request_id}");
        ...
        thread::Builder::new()
            .name("orbis-claude-process".into())
            .spawn(move || {
...
fn start_claude_task_output_tail(
    ...
    let spawned = thread::Builder::new()
        .name("orbis-claude-task-output".into())
        .spawn(move || {
...
fn handle_message(
    ...
    match value.get("type").and_then(Value::as_str) {
        ...
        Some("user") => {
            ...
            // `--replay-user-messages` echoes Orbis's own prompts back; they are
...
mod tests {
    ...
    fn native_task_output_streams_before_completion() {
        let root = std::env::temp_dir().join(format!("orbis-claude-tail-test-{}", Uuid::new_v4()));
    ...
    fn background_bash_task_keeps_its_originating_command() {
        ...
        handle_message(
            &json!({
                ...
                "message": {"content": [{
                    ...
                    "name": "Bash",
                    "input": {
                        "command": "cargo check && cargo test --bin orbis",
        ...
        assert_eq!(
            ...
            Some("cargo check && cargo test --bin orbis")
        );
    }
    ...
    fn streams_text_and_tools_and_ignores_its_own_replayed_prompt() {
        ...
        let wire = [
            ...
            // Orbis's own prompt, echoed by --replay-user-messages.
