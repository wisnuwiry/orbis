...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        ...
        serde_json::json!({
            ...
            "enabled": true,
            "environment": {
                "ORBIS_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "ORBIS_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "ORBIS_COMPUTER_USE_SERVER".to_owned(),
            base.server_path.display().to_string(),
        ),
        (
            "ORBIS_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
fn build_grok_computer_use_config(
    ...
    let rules = fs::read_to_string(&base.skill_path).with_context(|| {
        format!(
            "could not read Orbis Computer Use skill {}",
...
fn build_grok_computer_use_toml(
    ...
    environment.insert(
        "ORBIS_COMPUTER_USE_SERVER".into(),
        toml::Value::String(base.server_path.display().to_string()),
    );
    environment.insert(
        "ORBIS_COMPUTER_USE_PROCESS_DIRECTORY".into(),
    ...
    mcp_servers.insert("orbis_js_repl".into(), toml::Value::Table(server));
...
mod tests {
    ...
    fn computer_use_config() -> computer_use_runtime::ComputerUseConfig {
        computer_use_runtime::ComputerUseConfig {
            ...
            repl_path: PathBuf::from("/Applications/Orbis.app/Contents/Resources/orbis_js_repl"),
            skill_path: PathBuf::from(
                "/Applications/Orbis.app/Contents/Resources/skills/orbis-computer-use/SKILL.md",
            ),
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Orbis Computer Use"),
            Path::new("/Applications/Orbis.app/Contents/Resources/orbis_js_repl"),
            Path::new(
                "/Applications/Orbis.app/Contents/Resources/skills/orbis-computer-use/SKILL.md",
            ),
            Path::new("/tmp/orbis computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/orbis_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Orbis.app/Contents/Resources/orbis_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/orbis_js_repl/environment/ORBIS_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Orbis Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/orbis_computer_use").is_none());
    }

    #[test]
    fn grok_computer_use_config_preserves_existing_config_and_replaces_orbis_server() {
        ...
        let server = value
            .get("mcp_servers")
            .and_then(|mcp| mcp.get("orbis_js_repl"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            server
                .get("env")
                .and_then(|env| env.get("ORBIS_COMPUTER_USE_SERVER"))
                .and_then(toml::Value::as_str),
            Some("/tmp/Orbis Computer Use")
        );
    }
