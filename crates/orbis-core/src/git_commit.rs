...
pub use orbis_protocol::git::{AgentInvocation, CommitSnapshot as Snapshot};
...
pub fn generate_message(
    ...
    let amp_settings = if invocation.provider == ProviderKind::Amp {
        let path = std::env::temp_dir().join(format!("orbis-amp-commit-{}.json", Uuid::new_v4()));
        fs::write(
            &path,
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("orbis-commit-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        run_git(&root, &["init", "-b", "main"]);
        run_git(&root, &["config", "user.name", "Orbis Tests"]);
        run_git(&root, &["config", "user.email", "orbis@example.com"]);
