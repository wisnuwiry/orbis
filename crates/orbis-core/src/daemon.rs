//! Provider backend and driver-event wire translation for `orbis-daemon`.
...
use orbis_protocol::provider_session::{ProviderSessionFork, ProviderSessionForkRequest};
...
impl OrbisBackend {
    pub fn new(settings: DaemonSettingsStore, task_store: StateStore) -> anyhow::Result<Self> {
        let mut task_state = task_store
            .load()
            .context("could not load Orbis task database")?;
...
fn migrate_projectless_state(
    ...
    for index in indices {
        ...
        let workspace = crate::projectless::migrate_workspace(&old_path).with_context(|| {
            format!(
                "could not move projectless workspace {} under ~/.orbis/projects",
                old_path.display()
            )
        })?;
        ...
        match request.command {
            ...
            Command::Start { options } => {
                ...
                std::thread::Builder::new()
                    .name(format!("orbis-daemon-events-{session_id}"))
                    .spawn(move || {
...
mod tests {
    ...
    fn client_projection_cannot_replace_a_daemon_checkpoint() {
        ...
        let checkpoint = Checkpoint {
            turn_count: 1,
            git_ref: "refs/orbis/canonical".into(),
        ...
        incoming.turns[0].checkpoint = Some(Checkpoint {
            git_ref: "refs/orbis/stale-client".into(),
            ..checkpoint.clone()
        });
