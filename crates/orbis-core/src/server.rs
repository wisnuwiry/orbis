...
impl RequestDispatcher {
    ...
    fn dispatch_independent(
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("orbis-daemon-request".into())
            .spawn(move || {
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("orbis-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
...
pub fn serve(
    ...
    listener
        ...
        .context("could not configure Orbis daemon listener")?;
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("orbis-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("orbis-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start Orbis daemon connection thread")?;
            }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(ACCEPT_POLL_INTERVAL);
            }
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) => return Err(error).context("Orbis daemon listener failed"),
        }
    }
    backend.shutdown();
    Ok(())
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("orbis-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("Orbis daemon WebSocket failed"),
        }
    }
...
mod tests {
    ...
    use orbis_client::{DaemonClient, DaemonSupervisor};
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.orbis/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn stale_projection_cannot_resurrect_a_removed_session() {
        let root = std::env::temp_dir().join(format!("orbis-remove-race-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let backend = OrbisBackend::new(
    ...
    fn dropping_an_idle_terminal_does_not_wait_for_output() {
        let root = std::env::temp_dir().join(format!("orbis-terminal-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let hub = Arc::new(Hub::default());
    ...
    fn websocket_terminal_round_trip_streams_input_and_output() {
        ...
        client
            .request(
                ...
                terminal_id,
                Command::WriteTerminal {
                    data: b"orbis-terminal-round-trip\r".to_vec(),
                },
            )
            .unwrap();
        ...
        let marker = b"orbis-terminal-round-trip";
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.orbis.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.orbis.test".to_owned()]);
