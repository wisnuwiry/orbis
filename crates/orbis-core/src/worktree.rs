...
//! worktree beneath `~/.orbis/worktrees`, then every workspace consumer uses
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.orbis/worktrees"))?
        .join(".orbis/worktrees");
    create_in(
        project_path,
        &root,
        project_id,
...
fn create_in(
    ...
    let branch = format!("orbis/{fallback}");
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        let root = std::env::temp_dir().join(format!("orbis-worktree-test-{}", Uuid::new_v4()));
        ...
        .unwrap();
        assert_eq!(first.branch, "orbis/build-a-project-selector");
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(second.branch, "orbis/build-a-project-selector-2");
