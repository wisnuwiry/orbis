...
fn retained_turn_count(total_turns: usize, turns_to_remove: usize) -> anyhow::Result<usize> {
    total_turns.checked_sub(turns_to_remove).ok_or_else(|| {
        anyhow!(
            "OpenCode has only {total_turns} native turns, but Orbis needs to remove {turns_to_remove}"
        )
    })
}
...
fn fork_message_id(message_ids: &[String], retained_turns: usize) -> anyhow::Result<Option<&str>> {
    if retained_turns > message_ids.len() {
        bail!(
            "OpenCode has only {} native turns, but Orbis needs {retained_turns}",
            message_ids.len()
        );
    }
...
mod tests {
    ...
    /// submitted after Orbis has relaunched. The source session is supplied by
    ...
    #[test]
    #[ignore = "requires an installed opencode and ORBIS_OPENCODE_TEST_SESSION_ID"]
    fn forks_away_a_real_single_turn_session() {
        let binary =
            crate::command_env::find_executable("opencode").expect("opencode is not installed");
        let session_id = std::env::var("ORBIS_OPENCODE_TEST_SESSION_ID")
            .expect("set ORBIS_OPENCODE_TEST_SESSION_ID to a completed one-turn session");
