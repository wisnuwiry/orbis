        ...
        .filter_map(|entry| {
            ...
            if entry.get("supportsEffort").and_then(Value::as_bool) != Some(false) {
                model.reasoning_efforts = entry
                    ...
                    .collect();
                // `ultracode` is Orbis's orchestration effort. Claude accepts
...
fn discover_pi_models(binary: &Path, dialect: PiDialect) -> Vec<ProviderModel> {
    ...
    let models_request = json!({"id": "orbis-models", "type": "get_available_models"});
    let result = if write_json_line(&mut stdin, &models_request).is_ok()
        && let Some(models) = recv_pi_rpc_response(&rx, "orbis-models", PI_RPC_TIMEOUT)
    {
        ...
        let state_request = json!({"id": "orbis-state", "type": "get_state"});
        let state = if write_json_line(&mut stdin, &state_request).is_ok() {
            recv_pi_rpc_response(&rx, "orbis-state", PI_RPC_TIMEOUT).unwrap_or(Value::Null)
        } else {
            Value::Null
        };
...
fn grok_reasoning_model(model: ProviderModel) -> ProviderModel {
    match orbis_protocol::model_catalog::grok_model_reasoning_efforts(&model.id) {
...
mod tests {
    ...
    fn model_cache_round_trips_and_rejects_empty_or_invalid_files() {
        let directory =
            std::env::temp_dir().join(format!("orbis-model-cache-test-{}", std::process::id()));
    ...
    fn discovers_claude_models_from_sessionless_initialization() {
        let binary = write_fake_model_cli(
            ...
            r#"#!/bin/sh
read -r request
printf '%s\n' '{"type":"control_response","response":{"request_id":"orbis-initialize-catalog","response":{"models":[{"value":"cc-switch-model","resolvedModel":"cc-switch-model","displayName":"CC Switch Model"}]}}}'
"#,
        );
...
while IFS= read -r request; do
  case "$request" in
    *orbis-models*)
      printf '%s\n' '{"id":"orbis-models","type":"response","success":true,"data":{"models":[{"provider":"extension-provider","id":"extension-model","name":"Extension Model","reasoning":false}]}}'
      ;;
    *orbis-state*)
      printf '%s\n' '{"id":"orbis-state","type":"response","success":true,"data":{"model":{"provider":"extension-provider","id":"extension-model"},"thinkingLevel":"medium"}}'
      ;;
  esac
done
"#,
