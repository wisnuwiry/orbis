...
// Rust destructors. Keep one pipe open in Orbis and let this wrapper terminate
...
impl DeepSeekServer {
    ...
    fn start_with_dsh_home(binary: &Path, dsh_home: Option<&Path>) -> anyhow::Result<Self> {
        ...
        let mut command = {
            ...
            command
                .args(["-c", DSH_GUARDIAN_SCRIPT, "orbis-dsh-guardian"])
                .arg(binary)
        ...
        thread::Builder::new()
            .name("orbis-deepseek-stdout".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("orbis-deepseek-stderr".into())
            .spawn(move || {
        ...
        for (name, path) in [("mux", "/api/events.mux"), ("host", "/api/events.host")] {
            ...
            thread::Builder::new()
                .name(format!("orbis-deepseek-{name}-events"))
        ...
        thread::Builder::new()
            .name("orbis-deepseek-process-monitor".into())
            .spawn(move || {
                loop {
                    ...
                    match status {
                        Ok(Some(status)) => {
                            monitor_events.publish(json!({
                                "type": "server-request",
                                "rpcId": format!("orbis-process-{}", Uuid::new_v4()),
                                "method": "orbis/process-exited",
                                "payload": {
                                    "type": "orbis/process-exited",
    ...
    pub(crate) fn rpc(&self, method: &str, payload: Value) -> anyhow::Result<Value> {
        let rpc_id = format!("orbis-{}", Uuid::new_v4());
        let body = json!({
            "type": "client-request",
            "rpcId": rpc_id,
            "method": method,
...
fn stream_error(message: String) -> Value {
    json!({
        "type": "server-request",
        "rpcId": format!("orbis-stream-{}", Uuid::new_v4()),
        "method": "stream/error",
        "payload": {
            "type": "stream/error",
...
mod tests {
    ...
    /// Exercises Orbis's HTTP envelope, WebSocket handshakes, and process-tree
    ...
    fn installed_harness_host_answers_rpc() {
        ...
        #[cfg(unix)]
        {
            // Simulate an abruptly terminated Orbis process: the OS closes its
    ...
    fn installed_harness_session_contracts_round_trip() {
        ...
        let binary =
            crate::command_env::find_executable("dsh").expect("DeepSeek Harness is not installed");
        let root =
            TempHarnessHome(std::env::temp_dir().join(format!("orbis-dsh-test-{}", Uuid::new_v4())));
        std::fs::create_dir_all(&root.0).unwrap();
        {
            let server = DeepSeekServer::start_with_dsh_home(&binary, Some(&root.0))
                .expect("Harness Host should start");
            let session_id = format!("orbis-test-{}", Uuid::new_v4());
            ...
            let created = server
                ...
                .expect("session.create should accept Orbis's payload");
            assert_eq!(
            ...
            let history = server
                ...
                .expect("session.history should accept Orbis's payload");
