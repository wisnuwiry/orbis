...
use uuid::Uuid;
pub use orbis_protocol::settings::DaemonSettings;
        ...
        let (mut settings, write_current) = match fs::read(&path) {
            Ok(bytes) => match serde_json::from_slice(&bytes) {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "Orbis daemon moved invalid settings to {}: {error}",
                        backup.display()
                    );
...
mod tests {
    ...
    fn legacy_combined_settings_keep_only_daemon_fields() {
        let path = std::env::temp_dir().join(format!("orbis-settings-{}.json", Uuid::new_v4()));
        fs::write(
            &path,
    ...
    fn imports_daemon_fields_from_a_debug_combined_settings_file() {
        let directory = std::env::temp_dir().join(format!("orbis-settings-{}", Uuid::new_v4()));
