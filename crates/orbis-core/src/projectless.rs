...
//! random fallback. Orbis mirrors that layout beneath `~/.orbis/projects` so
...
fn workspace_root_slot() -> &'static RwLock<Option<PathBuf>> {
    ...
    ROOT.get_or_init(|| {
        RwLock::new(dirs::home_dir().map(|home| home.join(".orbis").join("projects")))
    })
}

pub fn set_workspace_root(root: Option<PathBuf>) {
    orbis_protocol::projectless::set_workspace_root(root.clone());
...
/// Existing builds created dated workspaces directly under `~/.orbis`; keep
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.orbis/projects",
        )
    })?;
    create_workspace_in(&root, Local::now().date_naive(), None, prompt)
}

/// Move one old dated workspace from `~/.orbis/<date>/<slug>` into
/// `~/.orbis/projects/<date>/<slug>` without copying its contents through the
/// client. The oldest layout used `~/.orbis` itself; that path contains Orbis's
...
mod tests {
    ...
    fn test_root() -> PathBuf {
        std::env::temp_dir().join(format!("orbis-projectless-{}", Uuid::new_v4()))
    }
