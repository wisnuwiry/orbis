...
pub use orbis_protocol::attachments::{
...
mod tests {
    ...
    fn imports_any_daemon_file_without_routing_bytes_through_the_client() {
        let source_directory =
            std::env::temp_dir().join(format!("orbis-file-source-{}", Uuid::new_v4()));
        let root = std::env::temp_dir().join(format!("orbis-attachments-{}", Uuid::new_v4()));
    ...
    fn daemon_path_import_follows_symbolic_links() {
        ...
        let outside = std::env::temp_dir().join(format!("orbis-outside-{}", Uuid::new_v4()));
        let root = std::env::temp_dir().join(format!("orbis-attachments-{}", Uuid::new_v4()));
