...
fn truncate_at_turn(
    ...
    if cutoff.is_none() {
        ...
        if turns < retained_turns {
            bail!("Grok has only {turns} native turns, but Orbis needs {retained_turns}");
        }
    }
...
fn write_atomic(
    ...
    let temp = parent.join(format!(".orbis-{}.tmp", Uuid::new_v4()));
...
mod tests {
    ...
    fn reads_groks_generated_title_from_native_metadata() {
        let root = std::env::temp_dir().join(format!("orbis-grok-title-{}", Uuid::new_v4()));
