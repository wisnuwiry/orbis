...
pub use orbis_protocol::composer::{CommandScope, FileEntry, SlashCommand};
...
/// - Codex: `~/.codex/prompts`, expanded by Orbis at submit.
/// - OpenCode: `.opencode/command` and `~/.config/opencode/command`, expanded
///   by Orbis — its server transport takes plain prompt text.
/// - Cursor: `.cursor/commands` in the project and home, expanded by Orbis.
/// - Pi: prompt templates in `.pi/prompts` and `~/.pi/agent/prompts`,
///   expanded by Orbis, plus skills in `.pi/skills` and `~/.pi/agent/skills`.
...
/// `/skill:name` there.
///
/// On top of provider sources, every provider reads Orbis's user-defined layer
/// (`.orbis/commands` and `~/.config/orbis/commands`).
...
fn assemble_slash_commands(
    ...
    match provider {
        ...
        // rather than from files Orbis could scan.
    ...
    // Orbis lists it for every provider.
    scan_skill_files(
        provider,
    ...
    scan_command_files(
        &project_root.join(".orbis/commands"),
        CommandScope::Project,
        true,
        &mut commands,
    );
    if let Some(home) = home.as_deref() {
        scan_command_files(
            &home.join(".config/orbis/commands"),
            CommandScope::User,
            true,
            &mut commands,
        );
    ...
    // `/resume` belongs to Orbis rather than any one provider. Reserve the
...
mod tests {
    ...
    fn provider_cli_catalog_entries_join_the_composer_index() {
        let root = std::env::temp_dir().join(format!("orbis-cli-catalog-{}", std::process::id()));
        ...
        let commands = assemble_slash_commands(
            ...
            &root,
            vec![SlashCommand {
                name: "orbis-test-dynamic-command".into(),
        ...
        let command = commands
            .iter()
            .find(|command| command.name == "orbis-test-dynamic-command")
    ...
    fn shared_skills_are_listed_raw_on_every_provider() {
        let root = std::env::temp_dir().join(format!("orbis-skills-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    #[test]
    fn orbis_resume_is_reserved_and_listed_for_every_provider() {
        let root = std::env::temp_dir().join(format!("orbis-resume-command-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join(".orbis/commands")).unwrap();
        std::fs::write(root.join(".orbis/commands/resume.md"), "Project override").unwrap();
    ...
    fn symlinked_skills_and_command_dirs_are_discovered() {
        let root = std::env::temp_dir().join(format!("orbis-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn codex_plugin_skill_uses_qualified_catalog_key() {
        let root =
            std::env::temp_dir().join(format!("orbis-codex-plugin-skill-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn derived_directories_join_the_file_index() {
        let root = std::env::temp_dir().join(format!("orbis-files-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
