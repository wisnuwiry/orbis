...
use orbis_protocol::composer::{CommandScope, SlashCommand};
use orbis_protocol::model::ProviderKind;
...
fn discover_codex(binary: &Path, project_root: &Path) -> Option<Vec<SlashCommand>> {
        ...
        && write_json_line(&mut stdin, &json!({"method": "initialized", "params": {}})).is_ok()
    {
        ...
        // the effective feature set backing the native commands Orbis bridges.
...
fn discover_pi(binary: &Path, project_root: &Path) -> Option<Vec<SlashCommand>> {
    ...
    // full-access flag.
    let request = json!({"id": "orbis-command-catalog", "type": "get_commands"});
    ...
    let value = probe_json_lines(
        ...
        CLI_PROBE_TIMEOUT,
        |value| {
            value.get("id").and_then(Value::as_str) == Some("orbis-command-catalog")
...
mod tests {
    ...
    fn parses_claude_initialization_metadata() {
        let commands = parse_claude_commands(&json!({
            ...
            "response": {
                "request_id": "orbis-command-catalog",
                "response": {"commands": [
