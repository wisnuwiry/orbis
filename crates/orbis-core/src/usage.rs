...
pub use orbis_protocol::usage::{PlanUsage, PlanWindow, format_tokens, reset_label};
...
pub fn fetch_grok_plan_usage(binary: &std::path::Path) -> anyhow::Result<PlanUsage> {
    ...
    let command = command
        .args(["agent", "stdio"])
        .env("GROK_OAUTH2_REFERRER", "orbis")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    let mut child =
...
fn grok_billing_over_stdio(child: &mut std::process::Child) -> anyhow::Result<Value> {
    ...
    std::thread::Builder::new()
        .name("orbis-grok-usage-probe".into())
        .spawn(move || {
