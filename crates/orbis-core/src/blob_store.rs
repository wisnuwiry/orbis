...
//! the transcript keeps only a `orbis-blob:` reference.
...
pub use orbis_protocol::blob::SCHEME as BLOB_SCHEME;
...
pub fn is_blob_reference(value: &str) -> bool {
    orbis_protocol::blob::is_reference(value)
}
...
impl BlobStore {
    ...
    /// Resolves a `orbis-blob:` reference against this daemon store's root.
...
mod tests {
    ...
    fn temporary_root() -> PathBuf {
        std::env::temp_dir().join(format!("orbis-blobs-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn traversal_references_are_rejected() {
        let store = BlobStore::new(temporary_root());
        assert!(store.path_for("orbis-blob:../../etc/passwd").is_none());
