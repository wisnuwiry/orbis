...
const TURN_START_METADATA_PREFIX: &str = "Orbis-Turn-Start: ";
...
pub fn capture_turn(cwd: &Path, session_id: Uuid, turn_count: usize) -> anyhow::Result<Checkpoint> {
    ...
    let end_commit =
        capture_worktree_commit_from(cwd, end_head.as_deref(), "Orbis worktree snapshot", &[])?;
...
pub fn capture_worktree_commit(cwd: &Path) -> anyhow::Result<String> {
    ...
    let head = resolve_ref(cwd, "HEAD");
    capture_worktree_commit_from(cwd, head.as_deref(), "Orbis worktree snapshot", &[])
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("orbis-checkpoint-index-{}", Uuid::new_v4()));
...
fn virtual_branch_start(
    ...
    commit_tree(cwd, tree, "Orbis turn diff base", &[])
}

fn empty_tree_commit(cwd: &Path) -> anyhow::Result<String> {
    commit_tree(cwd, EMPTY_TREE, "Orbis empty turn diff base", &[])
}
...
fn session_checkpoint_ref_commits(cwd: &Path, session_id: Uuid) -> SessionCheckpointRefs {
    let prefix = format!("refs/orbis/session-{session_id}-");
    git_output(
        cwd,
        [
            "for-each-ref",
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "Orbis")
            .env("GIT_AUTHOR_EMAIL", "orbis@localhost")
            .env("GIT_COMMITTER_NAME", "Orbis")
            .env("GIT_COMMITTER_EMAIL", "orbis@localhost");
    }
...
mod tests {
    ...
    fn diverged_repository() -> PathBuf {
        let directory = std::env::temp_dir().join(format!("orbis-checkpoints-{}", Uuid::new_v4()));
        fs::create_dir_all(&directory).unwrap();
        git_ok(&directory, &["init", "--quiet", "--initial-branch=main"]);
        git_ok(&directory, &["config", "user.name", "Orbis Test"]);
        git_ok(&directory, &["config", "user.email", "orbis@example.com"]);
    ...
    fn session_turn_refs_lists_the_sessions_checkpoints_in_one_call() {
        ...
        git_ok(
            ...
            &[
                "-c",
                "user.name=Orbis Test",
                "-c",
                "user.email=orbis@example.com",
                "commit",
                "--quiet",
                "-m",
                "baseline",
