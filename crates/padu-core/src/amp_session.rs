...
const CONTEXT_PREFIX: &str = "PADU_AMP_BRANCH_CONTEXT_V1 ";
const CONTEXT_GUIDANCE: &str = concat!(
    "\nPADU_AMP_BRANCH_INSTRUCTIONS_V1\n",
...
const PROMPT_PREFIX: &str = "PADU_AMP_CURRENT_PROMPT_V1 ";
...
fn retain_through_turn(messages: &[Value], retained_turns: usize) -> anyhow::Result<Vec<Value>> {
    ...
    if turns < retained_turns {
        bail!("Amp has only {turns} native turns, but Padu needs {retained_turns}");
    }
    Ok(retained)
}
