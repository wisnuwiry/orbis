...
//! `~/.padu/app.json` for Release, daemon preferences in
//! `~/.padu/settings.json`, and binary payloads in [`crate::blob_store`].
//! Of the configuration documents, only the desktop file is written here;
//! daemon settings cross the RPC boundary and are persisted by `padu-daemon`.
//!
...
/// at `~/.padu/app.json` without exposing app-managed state or daemon-owned
/// provider policy.
...
pub struct StateStore {
    ...
    /// Release uses the explicit cross-client Padu configuration directory.
...
impl StateStore {
    ...
    /// Local database owner used inside `padu-daemon`. It never reads or
...
mod tests {
    ...
    fn sent_attachment_presentation_round_trips_with_message_rows() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("padu-blob:abcdef.png".to_owned()),
        };
    ...
    fn default_path_is_build_specific() {
        ...
        {
            assert_eq!(directory, Some(std::ffi::OsStr::new("Padu")));
            let configuration_directory = dirs::home_dir()
                .unwrap_or_else(std::env::temp_dir)
                .join(".padu");
            assert_eq!(
