...
//! worktree beneath `~/.padu/worktrees`, then every workspace consumer uses
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.padu/worktrees"))?
        .join(".padu/worktrees");
    create_in(
        project_path,
        &root,
...
fn create_in(
    ...
    let branch = format!("padu/{fallback}");
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        let root = std::env::temp_dir().join(format!("padu-worktree-test-{}", Uuid::new_v4()));
        ...
        .unwrap();
        assert_eq!(first.branch, "padu/build-a-project-selector");
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(second.branch, "padu/build-a-project-selector-2");
