...
/// Oh My Pi refuses to reassemble beyond this, so neither should Padu.
...
impl PiFlavor {
    ...
    /// Oh My Pi does have one, and Padu only ever runs these in Full access.
    ...
    /// Padu's computer-use bridge is a Pi extension written against Pi's
    /// extension API. Oh My Pi ships its own `/computer` instead.
    fn supports_padu_computer_use(self) -> bool {
        matches!(self, Self::Pi)
    }
...
fn configure_pi_computer_use_command(
    ...
    if let Some((config, extension)) = config {
        command
            ...
            .env("PADU_JS_REPL_SERVER", &config.repl_path)
...
impl PiDriver {
    pub fn start(
        ...
        let computer_use = (computer_use_enabled && flavor.supports_padu_computer_use())
...
impl DriverControl for PiDriver {
    ...
    fn apply_options(&self, options: SessionOptions) -> bool {
        ...
        // Padu only runs them in Build with Full access anyway — so a mode
...
fn send_request(
    ...
    *next_request_id += 1;
    let id = format!("padu-{}", next_request_id);
...
/// Keep Padu's meter aligned with that provider-native calculation.
...
fn pi_fork_request(
    ...
    if turns_to_remove > messages.len() {
        return Err(format!(
            "{name} has only {} native turns, but Padu needs to remove {turns_to_remove}",
            messages.len()
        ));
    }
...
fn clone_ohmypi_session(
    ...
    let result = (|| {
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                for line in BufReader::new(stdout).lines().map_while(Result::ok) {
                    ...
                    if value.get("id").and_then(Value::as_str) == Some("padu-clone") {
                        let _ = tx.send(value);
                        break;
                    }
                }
        ...
        write_json_line(
            &mut stdin,
            &json!({"id": "padu-clone", "type": "get_state"}),
        )
...
mod tests {
    ...
    fn pi_computer_use_uses_only_session_scoped_extension_and_skill_arguments() {
        let config = computer_use_runtime::ComputerUseConfig {
            server_path: PathBuf::from("/tmp/Padu Computer Use"),
            repl_path: PathBuf::from("/Applications/Padu.app/Resources/padu_js_repl"),
            skill_path: PathBuf::from("/Applications/Padu.app/Resources/skills/SKILL.md"),
            process_directory: PathBuf::from("/tmp/padu-computer-use/session"),
        };
        ...
        configure_pi_computer_use_command(
            ...
            Some((
                &config,
                Path::new("/Applications/Padu.app/Resources/computer-use/pi-extension.ts"),
            )),
        );
        ...
        assert_eq!(
            ...
            [
                "--extension",
                "/Applications/Padu.app/Resources/computer-use/pi-extension.ts",
                "--skill",
                "/Applications/Padu.app/Resources/skills/SKILL.md",
            ]
        );
        let environment = command
            ...
            .collect::<HashMap<_, _>>();
        assert_eq!(
            environment.get("PADU_JS_REPL_SERVER"),
            Some(&Some(
                "/Applications/Padu.app/Resources/padu_js_repl".into()
            ))
        );
        assert_eq!(
            environment.get("PADU_COMPUTER_USE_PROCESS_DIRECTORY"),
            Some(&Some("/tmp/padu-computer-use/session".into()))
        );
    }
    ...
    fn chunked_frames_reassemble_and_reject_broken_runs() {
        ...
        let payload = json!({"type": "response", "id": "padu-1", "success": true});
        ...
        // A run that starts mid-sequence is not a frame Padu can trust.
        let mut assembly = ChunkAssembly::default();
