...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        ...
        serde_json::json!({
            ...
            "enabled": true,
            "environment": {
                "PADU_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "PADU_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "PADU_COMPUTER_USE_SERVER".to_owned(),
            base.server_path.display().to_string(),
        ),
        (
            "PADU_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
fn build_grok_computer_use_toml(
    ...
    environment.insert(
        "PADU_COMPUTER_USE_SERVER".into(),
        toml::Value::String(base.server_path.display().to_string()),
    );
    environment.insert(
        "PADU_COMPUTER_USE_PROCESS_DIRECTORY".into(),
    ...
    mcp_servers.insert("padu_js_repl".into(), toml::Value::Table(server));
...
mod tests {
    ...
    fn computer_use_config() -> computer_use_runtime::ComputerUseConfig {
        computer_use_runtime::ComputerUseConfig {
            ...
            repl_path: PathBuf::from("/Applications/Padu.app/Contents/Resources/padu_js_repl"),
            skill_path: PathBuf::from(
                "/Applications/Padu.app/Contents/Resources/skills/padu-computer-use/SKILL.md",
            ),
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Padu Computer Use"),
            Path::new("/Applications/Padu.app/Contents/Resources/padu_js_repl"),
            Path::new(
                "/Applications/Padu.app/Contents/Resources/skills/padu-computer-use/SKILL.md",
            ),
            Path::new("/tmp/padu computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/padu_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Padu.app/Contents/Resources/padu_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/padu_js_repl/environment/PADU_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Padu Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/padu_computer_use").is_none());
    }

    #[test]
    fn grok_computer_use_config_preserves_existing_config_and_replaces_padu_server() {
        ...
        let server = value
            .get("mcp_servers")
            .and_then(|mcp| mcp.get("padu_js_repl"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            server
                .get("env")
                .and_then(|env| env.get("PADU_COMPUTER_USE_SERVER"))
                .and_then(toml::Value::as_str),
            Some("/tmp/Padu Computer Use")
        );
    }
    ...
    fn grok_computer_use_command_is_process_scoped_and_loads_rules() {
        ...
        assert_eq!(
            ...
            Some(&"/tmp/padu-computer-use/session/grok-home".into())
        );
        assert_eq!(
