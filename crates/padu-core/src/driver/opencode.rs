...
//! permission requests the user can actually be asked. Padu already started
...
impl OpenCodeDriver {
    pub fn start(options: DriverStartOptions, events: DriverEventSender) -> anyhow::Result<Self> {
        ...
        // Padu must therefore install the selected access policy on this
        ...
        thread::Builder::new()
            .name("padu-opencode-usage-metadata".into())
            .spawn(move || {
        ...
        thread::Builder::new()
            .name("padu-opencode-events".into())
            .spawn(move || {
                ...
                match open_event_stream(stream_port, "/event", &stream_control) {
                    Ok(Some(stream)) => {
                        ...
                        // lost when Padu reconnects after it was asked. Events
        ...
        thread::Builder::new()
            .name("padu-opencode-driver".into())
            .spawn(move || {
...
fn rehydrate_pending_permissions(
    ...
    // `session.idle` settles Padu's persisted running turn exactly once.
...
fn request_permission(
    ...
    // so Padu sends only one-shot provider replies and retains durable choices
...
mod tests {
    ...
    /// `cargo test --bin padu opencode_session_against_a_real_server -- --ignored`.
    #[test]
