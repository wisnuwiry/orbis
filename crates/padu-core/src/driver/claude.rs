...
fn stop_task_request(request_id: u64, task_id: &str) -> Value {
    json!({
        ...
        "request_id": format!("padu-{request_id}"),
                    ...
                    let written = match message {
                        ...
                        CommandMessage::Cancel => {
                            ...
                            write_line(
                                ...
                                &json!({
                                    "type": "control_request",
                                    "request_id": format!("padu-{next_request_id}"),
                        ...
                        CommandMessage::StopBackgroundWork { key, control_id } => {
                            next_request_id += 1;
                            let request_id = format!("padu-{next_request_id}");
...
fn start_claude_task_output_tail(
    ...
    let spawned = thread::Builder::new()
        .name("padu-claude-task-output".into())
        .spawn(move || {
...
fn handle_message(
    ...
    match value.get("type").and_then(Value::as_str) {
        ...
        Some("user") => {
            ...
            // `--replay-user-messages` echoes Padu's own prompts back; they are
...
mod tests {
    ...
    fn locates_claudes_native_task_output_across_workspace_slugs() {
        let root = std::env::temp_dir().join(format!("padu-claude-output-test-{}", Uuid::new_v4()));
        let output = root
            .join("-Users-wisnu-dev-padu")
            .join("session-live")
            .join("tasks")
    ...
    fn native_task_output_streams_before_completion() {
        let root = std::env::temp_dir().join(format!("padu-claude-tail-test-{}", Uuid::new_v4()));
    ...
    fn background_bash_task_keeps_its_originating_command() {
        ...
        handle_message(
            &json!({
                ...
                "message": {"content": [{
                    ...
                    "name": "Bash",
                    "input": {
                        "command": "cargo check && cargo test --bin padu",
        ...
        assert_eq!(
            ...
            Some("cargo check && cargo test --bin padu")
        );
    }
    ...
    fn streams_text_and_tools_and_ignores_its_own_replayed_prompt() {
        ...
        let wire = [
            ...
            // Padu's own prompt, echoed by --replay-user-messages.
