    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("padu-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
...
pub fn serve(
    ...
    listener
        ...
        .context("could not configure Padu daemon listener")?;
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("padu-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("padu-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start Padu daemon connection thread")?;
            }
            Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                std::thread::sleep(ACCEPT_POLL_INTERVAL);
            }
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) => return Err(error).context("Padu daemon listener failed"),
        }
    }
    backend.shutdown();
    Ok(())
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("padu-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("Padu daemon WebSocket failed"),
        }
    }
...
mod tests {
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.padu/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn stale_projection_cannot_resurrect_a_removed_session() {
        let root = std::env::temp_dir().join(format!("padu-remove-race-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let backend = PaduBackend::new(
    ...
    fn dropping_an_idle_terminal_does_not_wait_for_output() {
        let root = std::env::temp_dir().join(format!("padu-terminal-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let hub = Arc::new(Hub::default());
    ...
    fn websocket_terminal_round_trip_streams_input_and_output() {
        ...
        client
            .request(
                ...
                terminal_id,
                Command::WriteTerminal {
                    data: b"padu-terminal-round-trip\r".to_vec(),
                },
            )
            .unwrap();
        ...
        let marker = b"padu-terminal-round-trip";
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.padu.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.padu.test".to_owned()]);
