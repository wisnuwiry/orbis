...
const TURN_START_METADATA_PREFIX: &str = "Padu-Turn-Start: ";
...
pub fn checkpoint_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/padu/session-{session_id}-turn-{turn_count}")
}

pub fn turn_start_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/padu/session-{session_id}-turn-start-{turn_count}")
}

pub fn turn_diff_base_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/padu/session-{session_id}-turn-diff-{turn_count}")
}
...
pub fn capture_turn(cwd: &Path, session_id: Uuid, turn_count: usize) -> anyhow::Result<Checkpoint> {
    ...
    let end_commit =
        capture_worktree_commit_from(cwd, end_head.as_deref(), "Padu worktree snapshot", &[])?;
...
pub fn capture_worktree_commit(cwd: &Path) -> anyhow::Result<String> {
    ...
    let head = resolve_ref(cwd, "HEAD");
    capture_worktree_commit_from(cwd, head.as_deref(), "Padu worktree snapshot", &[])
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("padu-checkpoint-index-{}", Uuid::new_v4()));
...
fn virtual_branch_start(
    ...
    commit_tree(cwd, tree, "Padu turn diff base", &[])
}

fn empty_tree_commit(cwd: &Path) -> anyhow::Result<String> {
    commit_tree(cwd, EMPTY_TREE, "Padu empty turn diff base", &[])
}
...
fn session_checkpoint_ref_commits(cwd: &Path, session_id: Uuid) -> SessionCheckpointRefs {
    let prefix = format!("refs/padu/session-{session_id}-");
    git_output(
        cwd,
        [
            "for-each-ref",
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "Padu")
            .env("GIT_AUTHOR_EMAIL", "padu@localhost")
            .env("GIT_COMMITTER_NAME", "Padu")
            .env("GIT_COMMITTER_EMAIL", "padu@localhost");
    }
...
mod tests {
    ...
    fn diverged_repository() -> PathBuf {
        let directory = std::env::temp_dir().join(format!("padu-checkpoints-{}", Uuid::new_v4()));
        fs::create_dir_all(&directory).unwrap();
        git_ok(&directory, &["init", "--quiet", "--initial-branch=main"]);
        git_ok(&directory, &["config", "user.name", "Padu Test"]);
        git_ok(&directory, &["config", "user.email", "padu@example.com"]);
