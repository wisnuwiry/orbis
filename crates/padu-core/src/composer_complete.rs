...
pub use padu_protocol::composer::{CommandScope, FileEntry, SlashCommand};
...
/// - Codex: `~/.codex/prompts`, expanded by Padu at submit.
/// - OpenCode: `.opencode/command` and `~/.config/opencode/command`, expanded
///   by Padu — its server transport takes plain prompt text.
/// - Cursor: `.cursor/commands` in the project and home, expanded by Padu.
/// - Pi: prompt templates in `.pi/prompts` and `~/.pi/agent/prompts`,
///   expanded by Padu, plus skills in `.pi/skills` and `~/.pi/agent/skills`.
...
/// `/skill:name` there.
///
/// On top of provider sources, every provider reads Padu's user-defined layer
/// (`.padu/commands` and `~/.config/padu/commands`).
...
fn assemble_slash_commands(
    ...
    scan_command_files(
        &project_root.join(".padu/commands"),
        CommandScope::Project,
        true,
        &mut commands,
    );
    if let Some(home) = home.as_deref() {
        scan_command_files(
            &home.join(".config/padu/commands"),
            CommandScope::User,
            true,
            &mut commands,
        );
    ...
    // `/resume` belongs to Padu rather than any one provider. Reserve the
...
mod tests {
    ...
    fn provider_cli_catalog_entries_join_the_composer_index() {
        let root = std::env::temp_dir().join(format!("padu-cli-catalog-{}", std::process::id()));
        ...
        let commands = assemble_slash_commands(
            ...
            &root,
            vec![SlashCommand {
                name: "padu-test-dynamic-command".into(),
        ...
        let command = commands
            .iter()
            .find(|command| command.name == "padu-test-dynamic-command")
    ...
    fn shared_skills_are_listed_raw_on_every_provider() {
        let root = std::env::temp_dir().join(format!("padu-skills-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    #[test]
    fn padu_resume_is_reserved_and_listed_for_every_provider() {
        let root = std::env::temp_dir().join(format!("padu-resume-command-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join(".padu/commands")).unwrap();
        std::fs::write(root.join(".padu/commands/resume.md"), "Project override").unwrap();
    ...
    fn symlinked_skills_and_command_dirs_are_discovered() {
        let root = std::env::temp_dir().join(format!("padu-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn codex_plugin_skill_uses_qualified_catalog_key() {
        let root =
            std::env::temp_dir().join(format!("padu-codex-plugin-skill-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn derived_directories_join_the_file_index() {
        let root = std::env::temp_dir().join(format!("padu-files-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
