...
pub use padu_protocol::git::{AgentInvocation, CommitSnapshot as Snapshot};
...
pub fn generate_message(
    ...
    let amp_settings = if invocation.provider == ProviderKind::Amp {
        let path = std::env::temp_dir().join(format!("padu-amp-commit-{}.json", Uuid::new_v4()));
        fs::write(
            &path,
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("padu-commit-test-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        run_git(&root, &["init", "-b", "main"]);
        run_git(&root, &["config", "user.name", "Padu Tests"]);
        run_git(&root, &["config", "user.email", "padu@example.com"]);
