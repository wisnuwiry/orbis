...
pub use padu_protocol::workspace::{
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("padu-workspace-{}", Uuid::new_v4()));
    ...
    fn directory_browser_lists_an_arbitrary_daemon_directory() {
        let directory =
            std::env::temp_dir().join(format!("padu-directory-browser-{}", Uuid::new_v4()));
    ...
    fn file_reader_keeps_the_complete_disk_content() {
        let root = std::env::temp_dir().join(format!("padu-editor-file-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        let content = "line\n".repeat(10_000);
