...
pub use padu_protocol::computer_use::{
...
fn helper_app_path() -> anyhow::Result<PathBuf> {
    ...
    let contents = macos
        .parent()
        .ok_or_else(|| anyhow!("Padu app bundle is malformed"))?;
    let app_name = executable
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or_else(|| anyhow!("Padu executable name is invalid"))?;
    let helper_name = format!("{app_name} Computer Use");
    let path = contents.join("Helpers").join(format!("{helper_name}.app"));
    if !path.is_dir() {
        bail!("Computer Use helper is missing from this Padu build")
    }
    Ok(path)
}
...
pub fn helper_display_name() -> String {
    host_executable_path()
        ...
        .unwrap_or_else(|| "Padu Computer Use".into())
}
...
pub fn js_repl_server_path() -> anyhow::Result<PathBuf> {
    ...
    let contents = macos
        .parent()
        .ok_or_else(|| anyhow!("Padu app bundle is malformed"))?;
    let path = contents.join("Resources").join("padu_js_repl");
    if !path.is_file() {
        bail!("Padu JavaScript REPL is missing from this Padu build")
    }
    Ok(path)
}
...
pub fn pi_extension_path() -> anyhow::Result<PathBuf> {
    ...
    if !path.is_file() {
        bail!("Padu Pi Computer Use extension is missing from this Padu build")
    }
    Ok(path)
}
...
/// responsible application. A helper launched from inside Padu's bundle is
/// therefore attributed to Padu even though the capture API runs in the
...
fn install_helper_app(source: &Path) -> anyhow::Result<PathBuf> {
    ...
    let install_root = application_support.join("Padu").join("Computer Use");
...
fn helper_install_matches(source: &Path, destination: &Path) -> anyhow::Result<bool> {
    ...
    let fingerprint = Path::new("Contents/Resources/.padu-helper-fingerprint");
...
pub fn skill_root_path() -> anyhow::Result<PathBuf> {
    ...
    if !path.join("padu-computer-use").join("SKILL.md").is_file() {
        bail!("Padu Computer Use skill is missing from this Padu build")
    }
    Ok(path)
}
...
fn host_executable_path() -> anyhow::Result<PathBuf> {
    std::env::var_os(crate::APP_EXECUTABLE_ENV)
        ...
        .map(PathBuf::from)
        .map(Ok)
        .unwrap_or_else(|| std::env::current_exe().context("Padu executable path is unavailable"))
}
