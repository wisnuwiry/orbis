...
impl ProviderKind {
    ...
    /// fork or truncation method. Neither can reproduce Orbis's "drop the last N
...
/// Orbis task. Provider history remains authoritative; unsupported native
...
pub enum DriverEvent {
    ...
    /// fresh Harness session may resolve its deployment default when Orbis did
...
mod tests {
    ...
    fn attachment_messages_keep_transport_and_visible_content_separate() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("orbis-blob:ab/reference.png".to_owned()),
        };
    ...
    fn activity_targets_are_normalized_when_events_arrive() {
        let cases = [
            (
                ActivityKind::FileRead,
                serde_json::json!({"input": {"file_path": "/tmp/orbis/src/app.rs"}}),
                "/tmp/orbis/src/app.rs",
            ),
            (
                ActivityKind::FileSearch,
            ...
            (
                ActivityKind::FileList,
                serde_json::json!({"arguments": {"directory": "/tmp/orbis/src"}}),
                "/tmp/orbis/src",
            ),
            (
            ...
            (
                ActivityKind::Search,
                serde_json::json!({"action": {"queries": ["Orbis GPUI"]}}),
                "Orbis GPUI",
            ),
            (
                ActivityKind::FileRead,
                serde_json::json!("/tmp/orbis/README.md"),
                "/tmp/orbis/README.md",
            ),
        ];
    ...
    fn projectless_projects_use_projects_root_and_recognize_legacy_paths() {
        ...
        let ordinary = Project::from_path(home.join("dev/orbis"));
    ...
    fn legacy_file_edit_details_are_promoted_to_arguments_and_metadata() {
        ...
        assert_eq!(activities[0].file_changes[0].path, "/tmp/orbis/README.md");
    ...
    fn legacy_file_tools_are_reclassified_and_gain_cached_targets() {
        ...
        cached.display_target = Some("/tmp/orbis/src/persisted.rs".into());
        session.transcript_blocks.push(TranscriptBlock {
            after_message: 0,
            turn_id: None,
            activities: vec![
                ActivityItem::new(
                    ...
                    ActivityKind::Search,
                    "read",
                    Some(r#"{"filePath":"/tmp/orbis/src/model.rs"}"#.into()),
                    true,
                ),
                ActivityItem::new(
                    None,
        ...
        assert_eq!(
            ...
            Some("/tmp/orbis/src/persisted.rs")
        );
        assert!(
            activities[..2]
                .iter()
