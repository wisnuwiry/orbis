...
use orbis_protocol::{DAEMON_TOKEN_ENV, DaemonReady, PROTOCOL_VERSION};

fn main() -> anyhow::Result<()> {
    let arguments = Arguments::parse(std::env::args().skip(1))?;
    let token =
        std::env::var(DAEMON_TOKEN_ENV).context("Orbis daemon authentication token is missing")?;
    ...
    let listener = TcpListener::bind(&arguments.bind)
        .with_context(|| format!("could not bind Orbis daemon to {}", arguments.bind))?;
    ...
    let task_path = orbis_core::persistence::StateStore::default_path();
    let settings = orbis_core::DaemonSettingsStore::open_with_legacy(
        orbis_core::DaemonSettings::default_path(),
        [task_path.with_file_name("settings.json")],
    )
    .context("could not load daemon settings")?;
    let task_store = orbis_core::persistence::StateStore::daemon(task_path);
    orbis_core::serve(
        listener,
        token,
        Arc::new(orbis_core::daemon::OrbisBackend::new(settings, task_store)?),
        shutdown,
        orbis_core::ServerOptions {
...
mod tests {
    ...
    fn parses_repeated_browser_origin_allowlist_entries() {
        ...
        assert_eq!(
            ...
            ["https://app.orbis.test", "http://localhost:3000"]
        );
