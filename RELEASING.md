...
Orbis ships signed in-app updates on macOS, Linux, and Windows. Releases live in
a **Cloudflare R2** bucket served at **`https://releases.orbis.sh`**. macOS uses
...
**This Mac already has the key** — Orbis signs with the same default-account
...
Sparkle tools (they land in `.orbis-cache/sparkle/<version>/bin` after any
...
To split Orbis onto its own key later: `generate_keys --account orbis`, put the
new public key in Info.plist, and pass `--account orbis` through to
...
profile with `--notary-profile` / `ORBIS_NOTARY_PROFILE`.

### 3. Cloudflare R2 bucket + domain  ← **still to do once**

1. Create the bucket **`orbis-releases`** (Cloudflare dashboard → R2 → Create
   bucket). The release script will not create it — a bucket-scoped API token
   can't.
2. Attach the custom domain **`releases.orbis.sh`** to the bucket (bucket →
   Settings → Custom Domains). This serves objects publicly at
   `https://releases.orbis.sh/<file>`.
3. Make sure the R2 API token behind the `r2` rclone remote covers this bucket
   ...
   for kero; if `rclone lsf r2:orbis-releases --s3-no-check-bucket` returns
...
- **Download link**: `https://releases.orbis.sh/Orbis-<version>.dmg`
...
- `orbis-<version>-x86_64-unknown-linux-gnu.tar.gz`
- `orbis-<version>-aarch64-unknown-linux-gnu.tar.gz`
- `appcast-linux-x86_64.xml`, `appcast-linux-aarch64.xml`
- `latest-linux.txt` — the version `install.sh` resolves "latest" to

Windows CI adds:

- `Orbis-<version>-x86_64-Setup.exe`
- `Orbis-<version>-aarch64-Setup.exe`
- `orbis-<version>-x86_64-pc-windows-msvc.zip` (portable)
- `orbis-<version>-aarch64-pc-windows-msvc.zip` (portable)
...
[`resources/windows/orbis.iss`](resources/windows/orbis.iss) through Inno Setup's
`ISCC`. The installer is **per-user** (`PrivilegesRequired=lowest`,
`%LOCALAPPDATA%\Programs\Orbis`) — no elevation, which is exactly what lets the
...
**Never change `AppId` in `orbis.iss`.** It is how Windows recognizes an
...
prefix, then `orbis-updater` swaps it after the app's normal quit saves and
...
distribution Orbis can start on (2.35 — Ubuntu 22.04, Debian 12, Fedora 36).
...
`https://orbis.sh/install.sh` — see [docs/linux.md](docs/linux.md).

Publishing that GitHub release (or running **Sync release** from Actions)
uploads the assets to the `orbis-releases` R2 bucket. Configure these repository
secrets first:

| Secret | Purpose |
| --- | --- |
| `ORBIS_ANALYTICS_ENDPOINT` | embedded in every desktop CI build |
| `ORBIS_ANALYTICS_WEBSITE_ID` | embedded in every desktop CI build |
| `ORBIS_SIGNING_IDENTITY` | Developer ID identity selector |
...
| `R2_BUCKET` | optional; defaults to `orbis-releases` |
...
| `--build-number <n>` / `ORBIS_BUILD_NUMBER` | derived | `CFBundleVersion` override |
| `ORBIS_R2_REMOTE` | `r2` | rclone remote name |
| `ORBIS_R2_BUCKET` | `orbis-releases` | R2 bucket |
| `ORBIS_DOWNLOAD_URL_PREFIX` | `https://releases.orbis.sh/` | base URL in the appcast |
| `ORBIS_HISTORY_COUNT` | `15` | recent archives pulled for delta generation |
| `ORBIS_NO_HISTORY=1` | — | skip pulling old archives (full updates only) |
| `SPARKLE_BIN` | the `.orbis-cache` copy | Sparkle tools directory |
...
- **Debug builds never update themselves.** `Updater::init` returns `None`
  ...
  itself with a production Orbis. Set `ORBIS_FORCE_UPDATER=1` to exercise the
  ...
  start the watcher with `ORBIS_PREVIEW_UPDATE=1`; the sidebar immediately
...
- **Orbis isn't sandboxed**, so Sparkle's XPC services are unnecessary;
...
- **Platform artifacts:** keep the bucket layout flat and platform-tagged by
  ...
  (`Orbis-<v>.dmg`, `Orbis-<v>.zip`, `appcast.xml`) must keep their URLs.
  Linux CI releases produce `orbis-<v>-<target>.tar.gz` with
  `scripts/bundle-linux.sh`, Windows CI produces `orbis-<v>-<target>.zip` with
  `scripts/bundle-windows.ts`, and both land in GitHub Releases, then R2 via
  the sync workflow. Windows also ships `Orbis-<v>-<arch>-Setup.exe`; each
