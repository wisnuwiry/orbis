...
description: Serve the Padu web app from your own daemon and reach it over your own LAN, VPN, reverse proxy, or tunnel.
nav: Web UI
order: 6
...
Padu's daemon can serve the browser web app itself, from the same address it already uses for the API. You don't need the hosted app at [app.padu.dev](https://app.padu.dev): point a browser at your own daemon and you get the full UI, connected to your own agents, on infrastructure you control.
...
```bash
padu daemon start --web-ui --listen 0.0.0.0:6767
```
...
1. **Set a password** so only authorized clients can connect:
   ...
   ```bash
   PADU_PASSWORD=my-secret padu daemon start --web-ui --listen 0.0.0.0:6767
   ```
...
2. **Allow your hostname** so the daemon's DNS-rebinding protection accepts requests for your domain:
   ...
   ```bash
   padu daemon start --web-ui --listen 0.0.0.0:6767 --hostnames ".example.com"
   ```
...
`PADU_TRUSTED_PROXIES` accepts the same comma-separated values:

```bash
PADU_TRUSTED_PROXIES=loopback,172.16.0.0/12 padu daemon start --web-ui
```
...
For the remote/relay path (driving a daemon through the Padu relay rather than a reverse proxy), the relay has its own public-vs-internal TLS settings, see [Security](/docs/security).
...
For the full threat model, relay encryption, and DNS-rebinding details, see [Security](/docs/security) and [SECURITY.md](https://github.com/wisnuwiry/padu/blob/main/SECURITY.md).

## Troubleshooting

- **Blank page or 404 at `/`.** The web UI isn't enabled. Start the daemon with `--web-ui` and confirm with `padu daemon status` that it's the daemon you're hitting.
...
- [CLI](/docs/cli), the `padu daemon` commands.
