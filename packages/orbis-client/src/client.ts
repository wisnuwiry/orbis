...
export class OrbisRpcError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "OrbisRpcError";
  }
}
...
/** Browser-safe client for Orbis's versioned JSON-over-WebSocket protocol. */
export class OrbisClient {
  ...
  constructor(options: OrbisClientOptions) {
  ...
  connect(): Promise<void> {
    ...
    if (this.state === "connecting") {
      return Promise.reject(new Error("Orbis client is already connecting"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("Orbis daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("Orbis daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("Orbis daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("Orbis daemon disconnected during handshake"));
        this.markDisconnected(new Error("Orbis daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for Orbis daemon"));
  ...
  disconnect(): void {
    this.rejectConnect?.(new Error("Orbis client disconnected"));
    ++this.connectionGeneration;
    const socket = this.socket;
    this.socket = undefined;
    this.markDisconnected(new Error("Orbis client disconnected"));
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("Orbis daemon is disconnected");
    }
    return this.socket;
  }
  ...
  private handleMessage(message: ServerMessage): void {
    if (message.type === "response") {
      ...
      else pending.reject(new OrbisRpcError(message.outcome.error.message));
      return;
    }
