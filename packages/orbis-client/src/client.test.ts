...
import { OrbisClient, OrbisRpcError, daemonUrl, type WebSocketLike } from "./client";
...
async function connect(client: OrbisClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("OrbisClient", () => {
  ...
  test("surfaces daemon errors", async () => {
    ...
    await expect(response).rejects.toBeInstanceOf(OrbisRpcError);
  });
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Orbis client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Orbis daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Orbis daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://orbis.example.test")).toBe("wss://orbis.example.test/v1");
});
