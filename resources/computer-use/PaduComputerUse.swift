...
private final class ComputerUseStatusItem {
    ...
    private func createItemIfNeeded() {
        ...
        item.length = 54
        item.button?.toolTip = "Padu Computer Use"
        self.item = item
    }
    ...
    private func updateMenu() {
        ...
        let menu = NSMenu()
        let title = NSMenuItem(title: "Padu Computer Use", action: nil, keyEquivalent: "")
...
enum HelperError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingPermission(let permission): "\(helperDisplayName) needs \(permission) access. Open Padu Settings > Computer Use to grant it."
        case .unauthorizedClient(let reason): "Computer Use rejected a request that did not come from a trusted Padu app: \(reason)"
        ...
        case .targetBlocked: "Padu does not allow computer control of that app."
...
struct PaduComputerUse {
            ...
            do {
                ...
                case "initialize":
                    result = [
                        ...
                        "serverInfo": ["name": "Padu Computer Use", "version": "1.0.0"],
                    ]
                case "tools/list":
    ...
    static func serveMCPBridge() async throws {
        ...
        if let processDirectory = ProcessInfo.processInfo.environment["PADU_COMPUTER_USE_PROCESS_DIRECTORY"],
...
private func readExactly(_ count: Int, from input: FileHandle) throws -> Data? {
    ...
    while data.count < count {
        ...
        if chunk.isEmpty {
            ...
            throw HelperError.ipc("the Padu connection closed mid-message")
        }
        data.append(chunk)
    }
    return data
...
private final class UnixListener {
    ...
    init() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("padu-computer-use", isDirectory: true)
...
private func connectedChannel(at path: String) throws -> (FileHandle, pid_t) {
    ...
    do {
        ...
        guard getsockopt(descriptor, SOL_LOCAL, LOCAL_PEERPID, &peerPID, &peerPIDSize) == 0 else {
            throw HelperError.ipc("could not identify the Padu process")
        }
...
private final class WindowCaptureSession {
    ...
    init(
        ...
        outputQueue = DispatchQueue(label: "dev.padu.computer-use.capture.\(window.windowID)")
...
private func post(_ event: CGEvent, to processID: pid_t) {
    ...
    // input to a global event tap. PID-targeted delivery is what lets Padu
