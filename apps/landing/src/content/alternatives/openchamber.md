...
description: Padu is an OpenChamber alternative for developers who want multiple agent harnesses, application plugins, and an Apache-2.0 codebase.
nav: OpenChamber
order: 52
---
...
Padu orchestrates many coding-agent harnesses from desktop, mobile, web, and the CLI. Open source under Apache-2.0.

![Padu desktop and mobile app](/hero-mockup.png)

## The main difference

OpenChamber builds its agent workflow around OpenCode. Padu runs OpenCode alongside Claude Code, Codex, Pi, ACP agents, and custom providers.

OpenChamber packages its web interface for iOS and Android with Capacitor. The interface runs inside WKWebView or Android WebView, with native integrations for notifications, secure storage, QR pairing, and widgets. Padu's mobile client is built with React Native and distributed through the App Store and Google Play.

## Architecture

Both tools can run a server on your workstation or another machine and connect from desktop, web, and mobile clients.

Padu's daemon launches each provider through its own native harness or through ACP. OpenChamber manages an OpenCode server and builds its agent workflow around OpenCode sessions.
...
Padu is multi-provider at the agent-harness layer. It runs Claude Code, Codex, OpenCode, and Pi natively, plus 30+ agents through its ACP catalog and any custom CLI agent. See [all supported providers](/agents).
...
OpenChamber adds multi-run comparisons, Fusion, and guided changes walkthroughs. Padu adds split panes and tabs, a native Files and Changes explorer, and a full pull-request workflow across its clients.

## Automation

Padu exposes workspace and agent operations through its CLI, TypeScript SDK, and MCP server. These interfaces can create workspaces, launch agents, follow progress, send messages, and manage schedules.
...
Both tools provide iOS and Android clients and support dictation and spoken replies. Padu uses a React Native mobile interface. OpenChamber packages its web interface with Capacitor and adds native integrations around the WebView.
...
See also: [Padu vs Conductor](/alternatives/conductor), [Padu vs Superset](/alternatives/superset), [Padu vs Happy Coder](/alternatives/happy-coder).
