      ...
      <div className="bg-background">
        <div className="p-6 md:px-32 md:pt-20 max-w-7xl mx-auto">
          ...
          <header className="space-y-4 max-w-2xl">
            <h1 className="text-3xl md:text-5xl font-medium tracking-tight">
              Every agent Padu supports
            </h1>
            <p className="text-white/70 text-lg leading-relaxed">
              Padu runs the native CLI for {AGENT_PAGES.length} coding agents — your skills, your
              config, your MCP servers, all intact. Drive any of them seamlessly.
            </p>
          </header>
        </div>
        ...
        <main className="px-6 md:px-32 pb-24 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {AGENT_PAGES.map((agent) => (
              <Link
                key={agent.slug}
                to={`/${agent.slug}`}
                className="block rounded-xl border border-white/10 bg-white/[0.02] p-5 hover:border-purple-500/40 hover:bg-purple-500/[0.04] transition-colors"
              >
          ...
          <p className="mt-10 text-sm text-white/50">
            ...
            in <code className="font-mono text-white/60">~/.padu/config.toml</code>.
          </p>
        </main>
