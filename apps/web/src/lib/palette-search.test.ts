...
describe('command palette search parity', () => {
  test('ranks contiguous and boundary matches above loose subsequences', () => {
    expect(fuzzyScore('orbis', 'Orbis daemon')!).toBeGreaterThan(
      fuzzyScore('orbis', 'open repository branch inspection server')!,
    )
    expect(fuzzyScore('od', 'Orbis daemon')).not.toBeNull()
    expect(fuzzyScore('missing', 'Orbis daemon')).toBeNull()
  })
