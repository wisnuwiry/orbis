...
export function OrbisApp() {
  ...
  useEffect(() => {
    window.localStorage.setItem('orbis.sidebarVisible', String(sidebarVisible))
  }, [sidebarVisible])

  useEffect(() => {
    if (sidebarWidth === DEFAULT_SIDEBAR_WIDTH) return
    const timer = window.setTimeout(() => {
      window.localStorage.setItem('orbis.sidebarWidth', String(Math.round(sidebarWidth)))
    }, 150)
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      window.localStorage.setItem('orbis.rightPanelWidth', String(Math.round(rightPanelWidth)))
    }, 150)
    return () => window.clearTimeout(timer)
  }, [rightPanelWidth])
    ...
    <div className="flex h-dvh w-full overflow-hidden bg-background">
      ...
      <main className="relative flex min-w-0 flex-1 flex-col">
        <TaskHeader
          ...
          session={activeSession}
          sidebarVisible={sidebarVisible}
          title={newTaskMode ? t('menu.new_task') : current ? displayTitle(current) : 'Orbis'}
        />
...
function readSidebarVisible(): boolean {
  ...
  return window.localStorage.getItem('orbis.sidebarVisible') !== 'false'
}
      ...
      <Popover.Portal>
        ...
        >
          <Popover.Popup
            ...
            className="orbis-popover-surface max-h-[420px] w-[min(300px,calc(100vw-24px))] overflow-y-auto rounded-[12px] p-2 text-popover-foreground outline-none"
            initialFocus={false}
            ref={popup}
            role="menu"
            onKeyDown={(event) => {
              ...
              {(item.key.kind !== 'subagent' || stoppable) && (
                <span className="relative grid size-6 shrink-0 place-items-center">
                  ...
                  {stoppable && (
                    ...
                    >
                      <OrbisIcon className="size-3 text-[var(--text-tertiary)]" name="stopFilled" />
                    </button>
                  )}
                </span>
              )}
...
function EnvironmentAction({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <OrbisIcon className="size-3.5 text-[var(--text-secondary)]" name={icon} />
      <span className="min-w-0 flex-1 truncate text-left">{label}</span>
      {trailing && <OrbisIcon className="size-[13px] text-[var(--text-tertiary)]" name={trailing} />}
    </button>
  )
}
  ...
  return (
    <div className="flex min-h-0 flex-1 items-center justify-center px-8 pb-12">
      <div className="max-w-sm text-center">
        <OrbisIcon className="mx-auto size-6 text-ring" name="sparkle" />
        ...
        <div className="mt-5 flex flex-col items-center gap-2">
          <Button className="rounded-full" onClick={onAddProject}>
            <OrbisIcon name="plus" /> {t('sidebar.add_project')}
          </Button>
          <Button
