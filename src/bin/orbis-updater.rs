...
fn main() {
    eprintln!("orbis-updater is only supported on Linux");
    std::process::exit(1);
}
...
mod linux {
    ...
    impl Handoff {
        fn validate(arguments: Arguments) -> anyhow::Result<Self> {
            ...
            if arguments.parent_pid != unsafe { libc::getppid() as u32 } {
                bail!("the update helper was not launched directly by Orbis");
            }
            ...
            {
                bail!("the staged install name does not belong to this Orbis prefix");
            }
    ...
    fn apply_update(handoff: &Handoff) -> anyhow::Result<()> {
        ...
        if let Err(error) = fs::rename(&handoff.install_dir, &backup) {
            let message = format!("could not preserve the current Orbis install: {error}");
        ...
        if let Err(error) = fs::rename(&handoff.staged_dir, &handoff.install_dir) {
            ...
            sync_directory(&handoff.parent);
            let message = format!("could not activate the staged Orbis install: {error}");
            record_error(handoff, &message);
            let _ = launch(&handoff.install_dir, None);
            bail!(message);
        }
        ...
        let mut replacement = match launch(&handoff.install_dir, Some(&ready_path)) {
            Ok(child) => child,
            Err(error) => {
                let message = format!("the updated Orbis build could not start: {error}");
                rollback(handoff, &backup, &message)?;
                bail!(message);
            }
        };
        ...
        match wait_for_relaunch(&mut replacement, &ready_path) {
            RelaunchState::Ready => {
                ...
                if let Err(error) = fs::remove_dir_all(&backup) {
                    eprintln!(
                        "Orbis updater: update succeeded but the rollback copy {} could not be removed: {error}",
                        backup.display()
                    );
                }
                sync_directory(&handoff.parent);
            ...
            RelaunchState::Exited(status) => {
                let message =
                    format!("the updated Orbis build exited before its window opened ({status})");
                rollback(handoff, &backup, &message)?;
                bail!(message)
            }
            RelaunchState::TimedOut => {
                ...
                eprintln!(
                    "Orbis updater: the new build stayed alive but did not acknowledge startup; retaining {}",
                    backup.display()
                );
                Ok(())
            }
    ...
    fn rollback(handoff: &Handoff, backup: &Path, message: &str) -> anyhow::Result<()> {
        ...
        if let Err(error) = fs::rename(backup, &handoff.install_dir) {
            ...
            return Err(error).context("could not restore the previous Orbis install");
        }
        sync_directory(&handoff.parent);
        let _ = fs::remove_dir_all(&failed);
        record_error(handoff, message);
        launch(&handoff.install_dir, None)
            .context("the previous Orbis build was restored but could not be relaunched")?;
        Ok(())
    }

    fn launch(prefix: &Path, ready_path: Option<&Path>) -> std::io::Result<Child> {
        let mut command = Command::new(prefix.join("bin/orbis"));
        command
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null());
    ...
    fn wait_for_relaunch(child: &mut Child, ready_path: &Path) -> RelaunchState {
        ...
        loop {
            ...
            match child.try_wait() {
                ...
                Ok(None) => {}
                Err(error) => {
                    eprintln!("Orbis updater: could not observe the relaunched app: {error}");
    ...
    fn validate_packaged_layout(prefix: &Path) -> anyhow::Result<()> {
        ...
        if !metadata.file_type().is_dir() {
            bail!("the Orbis prefix is not a real directory");
        }
        ...
        {
            bail!("the install is not marked as a Orbis-managed tarball");
        }
    ...
    mod tests {
        ...
        fn write_layout(prefix: &Path, value: &[u8]) {
            ...
            fs::create_dir_all(prefix.join("share/orbis")).unwrap();
        ...
        fn packaged_layout_requires_real_executables_and_marker() {
            ...
            let prefix = directory.join("orbis.app");
            write_layout(&prefix, b"old");
            validate_packaged_layout(&prefix).unwrap();
            fs::remove_file(prefix.join("bin/orbis-daemon")).unwrap();
        ...
        fn directory_swap_can_be_rolled_back_without_merging() {
            ...
            let install = directory.join("orbis.app");
            let staged = directory.join(".orbis.app.update-test");
            let backup = directory.join(".orbis.app.backup-test");
            write_layout(&install, b"old");
            write_layout(&staged, b"new");

            fs::rename(&install, &backup).unwrap();
            fs::rename(&staged, &install).unwrap();
            assert_eq!(fs::read(install.join("bin/orbis")).unwrap(), b"new");
            let failed = directory.join(".orbis.app.failed-test");
            fs::rename(&install, &failed).unwrap();
            fs::rename(&backup, &install).unwrap();
            assert_eq!(fs::read(install.join("bin/orbis")).unwrap(), b"old");
        ...
        fn full_handoff_waits_for_startup_and_removes_the_rollback_copy() {
            ...
            write_layout(&install, b"old");
            write_layout(&staged, b"new");
            let replacement = staged.join("bin/orbis");
            fs::write(
                &replacement,
                b"#!/bin/sh\nprintf 'ready\\n' > \"$ORBIS_UPDATE_READY_FILE\"\n",
            )
            .unwrap();
            ...
            let handoff = Handoff {
                ...
                prefix_name: "orbis.app".into(),
                parent_pid: std::process::id(),
            };

            apply_update(&handoff).unwrap();

            assert!(install.join("bin/orbis").is_file());
            assert_eq!(fs::read(install.join("bin/orbis-daemon")).unwrap(), b"new");
            assert!(
