...
description: Padu is an open source Claude Desktop alternative that runs on your machines without a required Padu account, telemetry, or cloud service.
nav: Claude Desktop
order: 55
---

# Padu vs Claude Desktop
...
Padu is an open source control plane that runs on machines you control. It does not require a Padu account, collect telemetry, or depend on a Padu cloud service. Connect directly from desktop, mobile, web, or the CLI, or use the optional end-to-end encrypted relay when the daemon is behind a firewall.

Padu does not upload or store your code. The relay cannot read your code, messages, or agent output. You can also self-host the daemon, web client, and relay.

## Architecture

Padu runs an independent daemon on your laptop, workstation, VM, home lab, or cloud machine. Its clients connect directly or through the optional end-to-end encrypted relay. The daemon launches your installed providers with their existing credentials, skills, MCP servers, and project configuration.
...
Both Claude Desktop and Padu are available on macOS, Windows, and Linux.
...
```bash
padu run --provider claude "implement OAuth"
padu run --provider codex --worktree refactor-auth "refactor auth"
...
| Mobile coding workflow       | Native Padu workspace on iOS and Android                       | Dispatch and Cowork in Claude mobile |
...
See also: [Padu vs Codex App](/alternatives/codex-app), [Padu vs OpenCode Desktop](/alternatives/opencode-desktop), [Padu vs Conductor](/alternatives/conductor).
