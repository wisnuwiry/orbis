...
How each session in the sidebar gets its name: who writes the title, how Padu
...
UI, on its own cheap model, as part of work it is doing anyway. Padu's job is
...
[`AgentSession`](../crates/padu-protocol/src/model.rs) carries two:
...
[`display_title`](../crates/padu-protocol/src/model.rs#L940) resolves them:
an explicit `title` first, then `auto_title`, then `DEFAULT_TITLE`
(`"New task"`, [model.rs:841](../crates/padu-protocol/src/model.rs#L841)).
...
2. **Polled from a native store.** The agent writes the title to disk (or to a
   ...
   Grok work this way.
3. **Generated by Padu.** Only Codex.
...
| --- | --- | --- | --- | --- |
| Claude Code | Claude (Haiku 4.5) | transcript JSONL on disk | poll from each prompt, plus turn end | [title_refresh.rs:53](../crates/padu-core/src/driver/title_refresh.rs#L53), [claude.rs:1300](../crates/padu-core/src/driver/claude.rs#L1300) |
| Codex CLI | **Padu** (`gpt-5.6-luna`, effort `low`) | second `codex app-server` process | first `turn/started`, new threads only | [codex.rs:636](../crates/padu-core/src/driver/codex.rs#L636) |
| Codex CLI | Codex | JSON-RPC stream | thread start/resume, `thread/name/updated` | [codex.rs:1612](../crates/padu-core/src/driver/codex.rs#L1612), [codex.rs:1735](../crates/padu-core/src/driver/codex.rs#L1735) |
| Amp | Amp | `amp threads list --json` subprocess | poll at turn end | [title_refresh.rs:53](../crates/padu-core/src/driver/title_refresh.rs#L53) |
| Grok Build | Grok | `summary.json` on disk | poll at turn end | [title_refresh.rs:53](../crates/padu-core/src/driver/title_refresh.rs#L53) |
| OpenCode | OpenCode | SSE stream | `session.updated` | [opencode.rs:894](../crates/padu-core/src/driver/opencode.rs#L894) |
...
`generate_session_title` to Sonnet while the turn stayed on Opus. Padu does not
...
[`claude_session::session_metadata`](../crates/padu-core/src/claude_session.rs#L27)
...
Padu writes when it forks a session — and it wins by being later in the file.

Padu reads it on two paths:

- [`start_claude_title_refresh`](../crates/padu-core/src/driver/claude.rs#L124),
  ...
  for a slow start.
- The turn-end read at [claude.rs:1295](../crates/padu-core/src/driver/claude.rs#L1295),
...
### Codex — the one Padu generates

The Codex app-server **persists** a thread name but never generates one; naming
is the client's job, and Codex Desktop does it client-side too. So Padu matches
that behavior in
[`generate_codex_title`](../crates/padu-core/src/driver/codex.rs#L1006): a
second, short-lived `codex app-server --stdio` process runs one ephemeral,
read-only turn.

- Model `gpt-5.6-luna` ([codex.rs:958](../crates/padu-core/src/driver/codex.rs#L958)),
...
[`amp_session::thread_title`](../crates/padu-core/src/amp_session.rs#L49) shells
...
`generated_title` field ([grok_session.rs:22](../crates/padu-core/src/grok_session.rs#L22)).
...
- **OpenCode** — `session.updated` → `properties./info/title`. Titles beginning
  with `"New session - "` are dropped ([opencode.rs:892](../crates/padu-core/src/driver/opencode.rs#L892)):
  that is OpenCode's own placeholder, and letting it through would replace
  Padu's prompt fallback with something strictly less useful.
- **Pi and Oh My Pi** — `/data/sessionName` at connect, then the stream event.
  ...
  ([pi.rs:69](../crates/padu-core/src/driver/pi.rs#L69)). Either may send an
...
([acp.rs:1305](../crates/padu-core/src/driver/acp.rs#L1305)): it emits one at
the start of the first turn and Padu forwards the `title` as `AutoTitleUpdated`,
no polling and no extra process.

What it sends, though, is **the first prompt verbatim** — the same content as
Padu's own fallback, arriving through a different door. Kimi files it in its
