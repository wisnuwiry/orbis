...
How each session in the sidebar gets its name: who writes the title, how Orbis
...
UI, on its own cheap model, as part of work it is doing anyway. Orbis's job is
...
[`AgentSession`](../crates/orbis-protocol/src/model.rs) carries two:
...
[`display_title`](../crates/orbis-protocol/src/model.rs#L940) resolves them:
an explicit `title` first, then `auto_title`, then `DEFAULT_TITLE`
(`"New task"`, [model.rs:841](../crates/orbis-protocol/src/model.rs#L841)).
A provider title therefore never overwrites a name the user typed.

### The local fallback

[`set_title_from_prompt`](../crates/orbis-protocol/src/model.rs#L964) takes the
...
2. **Polled from a native store.** The agent writes the title to disk (or to a
   ...
   [`NativeTitleRefresh`](../crates/orbis-core/src/driver/title_refresh.rs) on a
   backoff schedule, latching `RESOLVED` on the first hit. Claude, Amp, and
   Grok work this way.
3. **Generated by Orbis.** Only Codex.
...
| --- | --- | --- | --- | --- |
| Claude Code | Claude (Haiku 4.5) | transcript JSONL on disk | poll from each prompt, plus turn end | [title_refresh.rs:53](../crates/orbis-core/src/driver/title_refresh.rs#L53), [claude.rs:1300](../crates/orbis-core/src/driver/claude.rs#L1300) |
| Codex CLI | **Orbis** (`gpt-5.6-luna`, effort `low`) | second `codex app-server` process | first `turn/started`, new threads only | [codex.rs:636](../crates/orbis-core/src/driver/codex.rs#L636) |
| Codex CLI | Codex | JSON-RPC stream | thread start/resume, `thread/name/updated` | [codex.rs:1612](../crates/orbis-core/src/driver/codex.rs#L1612), [codex.rs:1735](../crates/orbis-core/src/driver/codex.rs#L1735) |
| Amp | Amp | `amp threads list --json` subprocess | poll at turn end | [title_refresh.rs:53](../crates/orbis-core/src/driver/title_refresh.rs#L53) |
| Grok Build | Grok | `summary.json` on disk | poll at turn end | [title_refresh.rs:53](../crates/orbis-core/src/driver/title_refresh.rs#L53) |
| OpenCode | OpenCode | SSE stream | `session.updated` | [opencode.rs:894](../crates/orbis-core/src/driver/opencode.rs#L894) |
| Pi | Pi | NDJSON stream | connect, `session_info_changed` | [pi.rs:472](../crates/orbis-core/src/driver/pi.rs#L472), [pi.rs:1214](../crates/orbis-core/src/driver/pi.rs#L1214) |
| Oh My Pi | Oh My Pi | NDJSON stream | connect, `session_info_update` | [pi.rs:472](../crates/orbis-core/src/driver/pi.rs#L472), [pi.rs:1214](../crates/orbis-core/src/driver/pi.rs#L1214) |
| DeepSeek | Harness | stream + projections | `session/title`, projection replay | [deepseek.rs:782](../crates/orbis-core/src/driver/deepseek.rs#L782), [deepseek.rs:1139](../crates/orbis-core/src/driver/deepseek.rs#L1139) |
| Kimi Code | Kimi (placeholder) | ACP stream | `session_info_update` | [acp.rs:1305](../crates/orbis-core/src/driver/acp.rs#L1305) |
...
`generate_session_title` to Sonnet while the turn stayed on Opus. Orbis does not
...
[`claude_session::session_metadata`](../crates/orbis-core/src/claude_session.rs#L27)
...
Orbis writes when it forks a session — and it wins by being later in the file.

Orbis reads it on two paths:

- [`start_claude_title_refresh`](../crates/orbis-core/src/driver/claude.rs#L124),
  ...
  for a slow start.
- The turn-end read at [claude.rs:1295](../crates/orbis-core/src/driver/claude.rs#L1295),
...
### Codex — the one Orbis generates

The Codex app-server **persists** a thread name but never generates one; naming
is the client's job, and Codex Desktop does it client-side too. So Orbis matches
that behavior in
[`generate_codex_title`](../crates/orbis-core/src/driver/codex.rs#L1006): a
second, short-lived `codex app-server --stdio` process runs one ephemeral,
read-only turn.

- Model `gpt-5.6-luna` ([codex.rs:958](../crates/orbis-core/src/driver/codex.rs#L958)),
...
[`amp_session::thread_title`](../crates/orbis-core/src/amp_session.rs#L49) shells
...
`generated_title` field ([grok_session.rs:22](../crates/orbis-core/src/grok_session.rs#L22)).
...
- **OpenCode** — `session.updated` → `properties./info/title`. Titles beginning
  with `"New session - "` are dropped ([opencode.rs:892](../crates/orbis-core/src/driver/opencode.rs#L892)):
  that is OpenCode's own placeholder, and letting it through would replace
  Orbis's prompt fallback with something strictly less useful.
- **Pi and Oh My Pi** — `/data/sessionName` at connect, then the stream event.
  ...
  ([pi.rs:69](../crates/orbis-core/src/driver/pi.rs#L69)). Either may send an
...
([acp.rs:1305](../crates/orbis-core/src/driver/acp.rs#L1305)): it emits one at
the start of the first turn and Orbis forwards the `title` as `AutoTitleUpdated`,
no polling and no extra process.

What it sends, though, is **the first prompt verbatim** — the same content as
Orbis's own fallback, arriving through a different door. Kimi files it in its
