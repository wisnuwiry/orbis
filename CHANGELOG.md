...
- Import and continue conversations started in agent CLIs with `/resume` or the command palette across every provider, in both Orbis and Orbis Web
- Linux: add signed in-app updates with clean relaunch and automatic rollback
- Copy Orbis task IDs and agent CLI thread IDs from task info or the command palette
...
- Codex thread goals: type /goal to set a persistent objective the task keeps pursuing — before or after the first message — with its autonomous pursuit streaming into the transcript, a status chip showing live budget or elapsed time, and a dialog to edit, pause, resume, or clear the goal (also in Orbis Web)
...
- Add standalone Orbis daemon and browser client
